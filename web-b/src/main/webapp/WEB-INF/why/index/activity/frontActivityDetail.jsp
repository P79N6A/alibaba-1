        ...
        <div class="the_one">
            <div class="a_time">
                <div class="time"> ${createTime} / 收藏：<span class="likeCount"></span> /
                    浏览：<span>${cmsActivity.yearBrowseCount==null?0:cmsActivity.yearBrowseCount}</span></div>
            </div>
            <div class="a_note">
                <div class="title">
			              	...
			              	<span>
				                ...
				                <c:if test="${not empty cmsActivity.activityReservationCount}"> ${cmsActivity.activityReservationCount}</c:if>
                                    ...
                                    <c:if test="${cmsActivity.activityReservationCount > 0}">
                                        <%--				               <c:if test="${cmsActivity.sysNo == '1'}">
                                                                        <a class="book-btn" style="margin-right:12px;" href="javascript:bookActivity('${cmsActivity.activityId}')">我要预订</a>
                                                                      </c:if>--%>
                                        <%-- <c:if test="${cmsActivity.sysNo=='0'||empty cmsActivity.sysNo}">--%>
                    ...
                    <p style="color: #F00;">
                        <c:if test="${cmsActivity.activityIsReservation == 1}">
                            <c:if test="${cmsActivity.activityIsFree == 1}">
                                温馨提示：直接前往、无需预约、无需付费，有任何问题欢迎拨打电话咨询（<c:out value="${cmsActivity.activityTel}"
                                                                         escapeXml="true"/>）
                            </c:if>
                            <c:if test="${cmsActivity.activityIsFree == 2}">
                                温馨提示：直接前往、无需预约、需要付费,有任何问题欢迎拨打电话咨询（<c:out value="${cmsActivity.activityTel}"
                                                                         escapeXml="true"/>）
                            </c:if>
                        </c:if>
                        <c:if test="${cmsActivity.activityIsReservation == 2}">
                            <c:if test="${cmsActivity.activityIsFree == 1}">
                                温馨提示：需要事先预定，请点击“我要预订”按钮进行票务预订。有任何问题欢迎拨打电话咨询（<c:out value="${cmsActivity.activityTel}"
                                                                                   escapeXml="true"/>）
                            </c:if>
                            <c:if test="${cmsActivity.activityIsFree == 2}">
                                温馨提示：需要事先预定，需要付费,请点击“我要预订”按钮进行票务预订。有任何问题欢迎拨打电话咨询（<c:out
