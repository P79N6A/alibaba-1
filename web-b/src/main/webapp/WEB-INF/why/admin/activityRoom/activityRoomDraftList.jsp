...
<head>
    ...
    <script type="text/javascript">
                ...
                dialogConfirm("提示", html, function(){
                    $.post("${path}/activityRoom/deleteActivityRoom.do",{"roomId":roomId}, function(data) {
                        if (data!=null && data=='success') {
                            window.location.href="${path}/activityRoom/roomDraftList.do";
                        }
                    });
                })
            });
        ...
        function publishActivityRoom(roomId){
            var html = "您确定要发布该活动室吗？";
            dialogConfirm("提示", html, function(){
                $.post("${path}/activityRoom/publishActivityRoom.do",{"roomId":roomId}, function(data) {
                    if (data!=null && data=='success') {
                        window.location.href="${path}/activityRoom/roomDraftList.do";
                    }
                });
...
<div class="site">
    <em>您现在所在的位置：</em>活动室信息管理 &gt; 活动室草稿箱
</div>
<form id="activityRoomIndexForm" action="${path}/activityRoom/roomDraftList.do" method="post">
    ...
    <table width="100%">
        ...
        <c:forEach items="${list}" var="c" varStatus="s">
            <tr>
                ...
                <td>
                <%
			      if(activityRoomDraftEditButton){
			     %>
                    <a href="${path}/activityRoom/preEditActivityRoom.do?roomId=${c.roomId}">编辑</a>|
			     <%
			        }
			     %>
			     <%
			      if(activityRoomDraftDeleteButton){
			     %>
			     ...
			     %>
			     <%
			      if(activityRoomSaveToDraftButton){
			     %>
                    <a href="javascript:;" onclick="publishActivityRoom('${c.roomId}')">发布</a>
                 <%
			        }
			     %>
                </td>
