...
<style type="text/css">
	.form-table .td-time .w240 {width: 208px!important;}
...
<script type="text/javascript">
	...
	$(function () {
	    $(".start-btn").on("click", function () {
	        WdatePicker({
	            el: 'startOrderDateHidden',
	            dateFmt: 'yyyy-MM-dd',
	            doubleCalendar: true,
	            minDate: '',
	            maxDate: '#F{$dp.$D(\'endOrderDateHidden\')}',
	            position: {left: -224, top: 8},
	            isShowClear: false,
	            isShowOK: true,
	            isShowToday: false,
	            onpicked: pickedStartOrderFunc
	        })
	    });
	    $(".end-btn").on("click", function () {
	        WdatePicker({
	            el: 'endOrderDateHidden',
	            dateFmt: 'yyyy-MM-dd',
	            doubleCalendar: true,
	            minDate: '#F{$dp.$D(\'startOrderDateHidden\')}',
	            position: {left: -224, top: 8},
	            isShowClear: false,
	            isShowOK: true,
	            isShowToday: false,
	            onpicked: pickedendOrderFunc
	        })
	    });
	    
	    $(".start-pay-btn").on("click", function () {
	        WdatePicker({
	            el: 'startOrderPayDateHidden',
	            dateFmt: 'yyyy-MM-dd',
	            doubleCalendar: true,
	            minDate: '',
	            maxDate: '#F{$dp.$D(\'endOrderPayDateHidden\')}',
	            position: {left: -224, top: 8},
	            isShowClear: false,
	            isShowOK: true,
	            isShowToday: false,
	            onpicked: pickedStartOrderPayFunc
	        })
	    });
	    
	    $(".end-pay-btn").on("click", function () {
	        WdatePicker({
	            el: 'endOrderPayDateHidden',
	            dateFmt: 'yyyy-MM-dd',
	            doubleCalendar: true,
	            minDate: '#F{$dp.$D(\'startOrderPayDateHidden\')}',
	            position: {left: -224, top: 8},
	            isShowClear: false,
	            isShowOK: true,
	            isShowToday: false,
	            onpicked: pickedendOrderPayFunc
	        })
	    })
	});
	function pickedStartOrderFunc() {
	    $dp.$('activityOrderStartTime').value = $dp.cal.getDateStr('yyyy-MM-dd');
	}
	function pickedendOrderFunc() {
	    $dp.$('activityOrderEndTime').value = $dp.cal.getDateStr('yyyy-MM-dd');
	}
	function pickedStartOrderPayFunc() {
	    $dp.$('activityOrderPayStartTime').value = $dp.cal.getDateStr('yyyy-MM-dd');
	}
	function pickedendOrderPayFunc() {
	    $dp.$('activityOrderPayEndTime').value = $dp.cal.getDateStr('yyyy-MM-dd');
	}
	...
	function exportOrderExcel() {
		...
		if (orderPaymentStatus == undefined || orderPaymentStatus == "支付状态") {
			orderPaymentStatus = "";
		}
		...
		if (activityIsFree == undefined || activityIsFree == "全部订单") {
			activityIsFree = "";
		}
		
		var activityOrderStartTime = $('#activityOrderStartTime').val();
		var activityOrderEndTime = $('#activityOrderEndTime').val();
		...
		location.href = "${path}/order/exportOrderExcel.do?searchKey="
				...
				+"&activityIsFree="+activityIsFree
				+"&activityOrderStartTime="+activityOrderStartTime
				+"&activityOrderEndTime="+activityOrderEndTime;
	}
	...
	function formSub(formName) {
		...
		if (orderPaymentStatus == "支付状态") {
			$("#orderPaymentStatus").val("");
		}
	...
	function refundSuccess(id) {
		dialogConfirm("操作退款", "您确定要操作完成退款吗？", removeParent);
		function removeParent() {
			$.post("${path}/order/refundSuccess.do", {
				'activityOrderId' : id
			}, function(data) {
				if (data == "success") {
...
<body>
	...
	<%--		<div class="select-box w135">
				...
				<div id="areaDiv" class="select-text" data-value="">全部区域</div>
			...
			<div class="form-table" style="float:left;">
	            <div class="td-time" style="margin-top: 0px;">
	                <div class="start w240" style="margin-left: 8px;">
	                    <span class="text">订单开始日期</span>
	                    <input type="hidden" id="startOrderDateHidden"/>
	                    <input type="text" id="activityOrderStartTime" name="activityOrderStartTime"
	                           value="${activity.activityOrderStartTime}" readonly/>
	                    <i class="data-btn start-btn"></i>
	                </div>
	                <span class="txt" style="line-height: 42px;">至</span>
	                <div class="end w240">
	                    <span class="text">订单结束日期</span>
	                    <input type="hidden" id="endOrderDateHidden"/>
	                    <input type="text" id="activityOrderEndTime" name="activityOrderEndTime" 
	                    	   value="${activity.activityOrderEndTime}" readonly/>
