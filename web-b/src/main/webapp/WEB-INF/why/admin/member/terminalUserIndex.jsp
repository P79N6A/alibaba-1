...
<form action="${path}/terminalUser/terminalUserIndex.do" id="terminalUserForm" method="post">
	...
	<div class="search">
		...
		<div class="search-box">
			<i></i><input class="input-text" data-val="输入用户编号" name="userId" value="<c:if test="${not empty user.userId}">${user.userId}</c:if><c:if test="${empty user.userId}">输入用户编号</c:if>" type="text" id="userId"/>
		</div>
