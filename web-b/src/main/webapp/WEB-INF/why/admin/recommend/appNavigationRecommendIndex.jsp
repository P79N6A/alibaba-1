...
<head>
    ...
    <script type="text/javascript">
        ...
        function recommendActivity(activityId){
            $.ajax({
                type:"post",
                url:"${path}/recommend/appHomeNavigationRecommendActivity.do",
                dataType: "json",
                data:{activityId:activityId,activityTheme:$("#activityTheme").val(),activityArea:$("#activityArea").val(),activityThemeTxt:$("#activityThemeTxt").val()},
                cache:false,//缓存不存在此页面
                async: false,//异步请求
                success: function (result) {
                    if(result == "success"){
                        dialogAlert("提示", "置顶成功!",function () {
                            location.href= "${path}/recommend/appActivityHomeRecommendIndex.do?activityTheme="+$("#activityTheme").val()+"&activityArea="+$("#activityArea").val();
                        });
                    }else{
                        dialogAlert("提示", "置顶失败!");
                    }
        ...
        function cancelRecommendActivity(recommendId){
            $.ajax({
                type:"post",
                url:"${path}/recommend/cancelAppHomeNavigationRecommendActivity.do",
                dataType: "json",
        ...
        function formSub(formName){
            var  activityName=$('#activityName').val();
            var activityTheme = $("#activityTheme").attr("data-txt");
            if(activityName!=undefined&&activityName=='输入关键词'){
                $('#activityName').val("");
            }

            if(activityTheme != undefined && activityTheme != null && activityTheme != ""){
                $("#activityThemeTxt").val(activityTheme);
            }
            //场馆
...
<body>
<form id="activityForm" action="${path}/recommend/appActivityHomeRecommendIndex.do" method="post">
...
<div class="search">
    ...
    <input type="hidden" id="activityThemeTxt" name="activityThemeTxt" value="${activity.activityThemeTxt}"/>
                        ...
                        <c:choose>
                            <c:when test="${empty activity.activityName && status.index < 3 && page.page == 1}">
                                <c:if test="${not empty avct.activityRecommendTime}">
                                    <% if(appCancleRecommendActivityButton){
                                    %>
                                    <a target="main" onclick="cancelRecommendActivity('${avct.recommendId}')">取消置顶</a>
                                    <%
                                        }
                                    %>
                                </c:if>
