    ...
    <script type="text/javascript">
        ...
        $(function () {
         ...
         //   audiojs.events.ready(function () {
          ...
          //  }); 
        			...
        			venueTel = '${cmsVenue.venueMobile}';
                    lat = '${cmsVenue.venueLat}';
                    lon = '${cmsVenue.venueLon}';
                    var venueIconUrl = '${cmsVenue.venueIconUrl}';
                    
                    if(venueIconUrl)
                    {
                    	 $("#venueImg").attr("src", getIndexImgUrl(venueIconUrl, "_750_500"));
                    }
                 
                    var tagNames = '${cmsVenue.tagName}'.split(",");
                    
        			$.each(tagNames,function(i,tagName){
        				$("#tagName").append("<li>"+tagName+"</li>");
        				
        				if(i==2)
        					return false;
        			});
                   
                    var venueIsFree='${cmsVenue.venueIsFree}';
                    var venuePrice='${cmsVenue.venuePrice}';
                    
                    if(venueIsFree==2){
    					$("#venuePrice").html("收费");
    					if(venueIsFree.length != 0){
    						$("#venuePriceLi").show();
    						if (venuePrice > 0) {
        						$("#venuePriceDetail").html(venuePrice+"元/人");
    	                    }else{
        						$("#venuePriceDetail").html(venuePrice);
    	                    }
    					}
    				}else{
    					$("#venuePrice").html("免费");
                    ...
                    $("#venueName").html('${cmsVenue.venueName}');
                    $("#venueAddress").html('${cmsVenue.venueAddress}');
                    $("#venueOpenTime").html('${cmsVenue.venueOpenTime}');
                    if ('${cmsVenue.venueEndTime}'.length != 0) {
                        $("#venueOpenTime").append("&nbsp;-&nbsp;"+'${cmsVenue.venueEndTime}');
                    }
                    if('${cmsVenue.openNotice}'.length != 0){
                    	$("#venueOpenTime").append("</br>"+'${cmsVenue.openNotice}');
                    }
                    $("#venueMobile").html('${cmsVenue.venueMobile}');
                    if ('${cmsVenue.venueVoiceUrl}'.length > 0) {
                        $("#venueVoiceLi").show();
                        $("#venueVoice").attr("data-href", '${cmsVenue.venueVoiceUrl}');
                    }
        ...
        function addComment() {
            if (userId == null || userId == '') {
                window.location.href = '${path}/muser/login.do?type=${path}/wechatVenue/venueDetailIndex.do?venueId=' + venueId;
                return;
            } else {
                ...
                if (status == 2) {
                    ...
                    window.location.href = "${path}/wechat/preAddWcComment.do?moldId=" + venueId + "&type=1";
                }
            }
        }
                    ...
                    $.each(data.data, function (i, dom) {
	    				...
	    				if(dom.activityIsFree==2){
	    					if (dom.activityPrice.length != 0 && dom.activityPrice > 0) {
		    					if(dom.priceType==0){
		    						price = "<div class='venue-pay'><span>"+dom.activityPrice+"元起</span></div>";
	    						}else{
	    							price = "<div class='venue-pay'><span>"+dom.activityPrice+"元/人</span></div>";
	    						}
		                    } else {
		                    	price = "<div class='venue-pay'><span>收费</span></div>";
		                    }
	    				}else{
	    					price = "<div class='venue-pay'><span>免费</span></div>";
	    				}
                    ...
                    $.each(data.data, function (i, dom) {
                        if (dom.userHeadImgUrl.indexOf("http") == -1) {
                            ...
                            if (ImgObj.fileSize > 0 || (ImgObj.width > 0 && ImgObj.height > 0)) {
                                $("#wantGoList").append("<li><img src='" + smallUrl + "' width='65' height='65' onerror='imgNoFind();'></li>");
                            } else {
                                $("#wantGoList").append("<li><img src='" + bigUrl + "' width='65' height='65' onerror='imgNoFind();'></li>");
                            }
                        } else {
		...
		<div class="content" style="padding-bottom: 100px;">
			...
			<div class="active-detail">
				<ul>
					<li id="aboutActivity" style="display: none;">
						<div class="active-border">
							<div class="active-detail-p5">
								<h1 class="border-bottom active-p4-arrowr" onclick="window.location.href='${path}/wechatActivity/preActivityList.do?venueId=${venueId}'">场馆活动</h1>
					...
					<c:if test="${!empty cmsVenue.venueMemo}">
					<li id="cgDetail" >
						<div class="active-border active-tab">
							<div class="active-detail-p1 active-detail-p1-hide">
								<div class="active-detail-p1-show">
									...
									<p id="venueMemo">${cmsVenue.venueMemo}</p>
								</div>
							</div>
					...
					<li id="aboutRoom" style="display: none;">
						<div class="active-border">
							<div class="active-detail-p5">
								<h1 class="border-bottom active-p4-arrowr" onclick="window.location.href='${path}/wechatRoom/preRoomList.do?venueId=${venueId}'">活动室</h1>
								<p class="p3-right" style="margin-right: 30px;line-height: 30px;">
