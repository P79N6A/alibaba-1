...
<head>
  ...
  <script type="text/javascript">
    ...
    //删除
    function culDelete(id){

          dialogConfirm("提示","确定要删除吗？ 删除后无法恢复!",function(){
                    $.ajax({
                      type: "POST",
                      url: "${path}/culture/delete.do",
                      data: {
                        id: id
                      },
                      dataType: "json",
    ...
    function updateState(id,state){
          ...
          dialogConfirm("提示","确定要还原吗？",function(){
                  $.ajax({
                      type: "POST",
                      ...
                      success: function (data) {
                              ...
                              if (data=="success") {
                                  dialogSaveDraft("提示", "<h2>已还原到草稿箱!</h2>", function(){
                                    toList();
                                  });
                              }else{
                                  dialogSaveDraft("提示", "<h2>还原失败!</h2>", function(){
                                    toList();
                                  });
                              }
                      }
    ...
    <table width="100%">
      ...
      <c:forEach items="${dataList}" var="c" varStatus="s">
        <tr>
          ...
          <td>
            <a href="javascript:;" onclick="culDelete('${c.cultureId}')">删除</a>|
            <a href="javascript:;" onclick="updateState('${c.cultureId}','2')">还原</a>
          </td>
