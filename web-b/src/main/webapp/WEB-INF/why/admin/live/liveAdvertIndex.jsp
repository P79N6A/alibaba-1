...
<script type="text/javascript">
    ...
    seajs.use(['jquery'], function ($) {
        ...
        $('.btn-add-tag').on('click', function () {
            ...
            dialog({
                url: '${path}/live/addAdvertIndex.do?advertSort=' + sortNum,
                title: '添加首页轮播图',
                width: 1000,
                height: 800,
                fixed: true
    ...
    function advEdit(ID) {
        dialog({
            url: '${path}/live/addAdvertIndex.do?advertId=' + ID,
            title: '添加首页轮播图',
            width: 1000,
            height: 800,
            fixed: true
    ...
    function editAdvert(ID,advertState) {
        $.post("../live/editAdvert.do", {advertState:advertState,advertId:ID},
                function (data) {
                    switch (data) {
                        case("success"):
                            dialogAlert("系统提示", "操作成功", function () {
                                window.location.href = "../live/liveAdvertRecommendIndex.do";
                                dialog.close().remove();
                            });
                            break;
                        case("noLogin"):
...
<form id="advertIndexForm" action="" method="post">
                ...
                <c:forEach var="i" begin="1" end="6">
                    ...
                    <c:forEach items="${list}" var="advert">
                        <c:if test="${advert.advertSort eq i}">
                            <tr>
                                <td class="sortNum">${i}${advert.advertId}</td>
