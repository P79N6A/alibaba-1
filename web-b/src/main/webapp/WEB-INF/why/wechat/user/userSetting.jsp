...
<script type="text/javascript">
	...
	$(function(){
        ...
        $("body").on("click",".sex_sel label",function(){
			...
			$.post("${path}/wechatUser/editTerminalUser.do",data, function(data) {
				if(data.status==0){
					$("#set_sex").text(vals);
		...
		$.post("${path}/wechatUser/queryTerminalUserById.do",{userId:userId}, function(data) {
			if(data.status==0){
				...
				if(user.userHeadImgUrl.indexOf("http") == -1){
					...
					var smallUrl = getIndexImgUrl(user.userHeadImgUrl, "_72_72");
					var bigUrl = getIndexImgUrl(user.userHeadImgUrl, "_300_300");
					var ImgObj = new Image();
					ImgObj.src = smallUrl;
					if (ImgObj.fileSize > 0 || (ImgObj.width > 0 && ImgObj.height > 0)) {
						$("#userHeadImgUrl").attr("src",smallUrl);
					} else {
						$("#userHeadImgUrl").attr("src",bigUrl);
					}
				} else {
		        ...
		        wx.uploadImage({
		            ...
		            success: function (res) {
		                ...
		                $.post("${path}/wechat/wcUpload.do",{mediaId:serverId,userId:userId,uploadType:3}, function(data) {
		                	if(data!=1){
		                		$.post("${path}/wechatUser/editTerminalUser.do",{userId:userId,userSex:0,userHeadImgUrl:data}, function(data) {
		            				if(data.status==0){
...
<style>
	.content{padding-bottom: 0px;padding-top: 100px;}
</style>
	...
	<div class="main">
		<div class="header">
			<div class="index-top">
				...
				<span class="index-top-2">个人设置</span>
			</div>
			...
			<div class="personal-p3">
				<ul>
					<li class="border-bottom" id="editPassLi" style="display: none;">
						<div class="personal-p3-bg" onclick="window.location.href='${path}/muser/preSetPass.do?userId=${sessionScope.terminalUser.userId}'">
							<label>修改密码</label>
							<span>修改</span>
							<div style="clear: both;"></div>
						</div>
