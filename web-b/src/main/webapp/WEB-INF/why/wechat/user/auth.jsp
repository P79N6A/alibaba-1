...
<script>
	...
	function sendCode()
	{
	   ...
	   $.post("${path}/wechatUser/sendAuthCode.do", {userId: userId,userTelephone: userTelephone}, function (data) {
    		if(data.status==0){
    			var s = 60;
            ...
            button: [{
                ...
                callback: function () {
                	if(!tuserIsDisplay && !roomOrderId){
    					...
    					if('${type}'=='app'){
    						location.href = '${path}/wechatRoom/authTeamUser.do?roomOrderId='+roomOrderId+"&userId="+userId+"&tuserId="+tuserId+"&type=app";
    					}else{
    						location.href = '${path}/wechatRoom/authTeamUser.do?roomOrderId='+roomOrderId+"&userId="+userId+"&tuserId="+tuserId;
    					}
    				}else{
    					if (/wenhuayun/.test(ua)) {		//APP端
			...
			<div class="content padding-bottom0">
						...
						<tbody>
							<tr class="border-bottom">
								...
								<c:choose>
									...
									<input placeholder="请输入真实姓名" type="text" name="nickName" id="nickName" value="${user.userNickName}" />
									</c:when>
									<c:otherwise>
									${user.userNickName}
									</c:otherwise>
							...
							<tr class="border-bottom">
								...
								<c:choose>
									<c:when test="${userType!=2 }">
									<input placeholder="请输入手机号" type="text" value="${user.userTelephone}" id="userTelephone" name="userTelephone" maxlength="11"/>
									</c:when>
									<c:otherwise>
									${user.userTelephone}
									</c:otherwise>
							...
							<tr class="border-bottom">
								...
								<td>:</td>
								<td><input placeholder="请输入6位验证码" type="text" name="code" id="code" value="" /></td>
								<td style="padding: 10px 0px;width: 200px;">
									<button id="smsCodeBut" class="certification-send" type="button" onclick="sendCode();">发送验证码</button>
								</td>
							</tr>
							</c:if>
							<tr>
								...
								<c:choose>
									<c:when test="${userType!=2 }">
									<input placeholder="请输入邮箱地址" type="text" name="userEmail" id="userEmail" value="${user.userEmail}" class="w500"/>
									</c:when>
									<c:otherwise>
									${user.userEmail}
									</c:otherwise>
				...
				<div class="certification bgfff fs30 margin-top20 padding-top20">
					<div class="border-bottom padding-bottom20">
						...
						<c:choose>
							<c:when test="${userType!=2 }">
							<input placeholder="请输入身份证号" style="border: none;" class="f-left c808080 w500" type="text" name="userCardNo" id="userCardNo" value="${user.userCardNo }" />
							</c:when>
							<c:otherwise>
