	...
	$(function(){
			...
			$.post("${path}/wechatRoom/roomWcDetail.do",{roomId:roomId}, function(data) {
				if(data.status==0){
					...
					//$("#roomPrice").html("费用："+price);
				//	if(dom.roomRemark.length>0){
					//	$("#roomRemark").show();
						$("#roomRemark").append(dom.roomRemark);
				//	}
					//var roomFacilityHtml = "";
					//if(dom.facility.length>0){
					//	$("#roomFacilityLi").show();
						...
						//$("#roomFacility").append(roomFacilityHtml);
					//}
					if(data.data1){	
						var dom1 = data.data1;
