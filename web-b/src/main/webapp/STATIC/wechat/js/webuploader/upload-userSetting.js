jQuery(function() {
	...
	uploader.on( 'uploadSuccess', function( file, response ) {
		$("#userHeadImgUrl").addClass('upload-state-done');
	    
	    //回调参数，加载图片
	    if(response.status==0){
	    	var imgUrl = response.data;
    		var index=imgUrl.indexOf("front");
    		imgUrl = imgUrl.substring(index,imgUrl.length);
    		$.post("../wechatUser/editTerminalUser.do",{userId:userId,userSex:0,userHeadImgUrl:imgUrl}, function(data) {
				if(data.status==0){
					$("#userHeadImgUrl").attr("src",getIndexImgUrl(response.data,"_150_150"));
				}
    		},"json");
	    }
	});
