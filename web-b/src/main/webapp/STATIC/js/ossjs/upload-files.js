...
function get_dirname_Files(Files_div) {
	dir = Files_div //document.getElementById("dirname").value;
...
function aliUploadFiles(uploadDomId, callback, upNum, auto, imgSrc) {
	if(!imgSrc) {
		imgSrc = 'H5'
	}
		...
		init: {
			...
			FilesAdded: function(up, files) {
				...
				plupload.each(files, function(file) {
					...
					$("#" + uploadDomId + " #ossfile2").append('<div name="aliFile" style="position:relative" id="' + file.id + '"><span>' + file.name + ' (' + plupload.formatSize(file.size) + ')</span><b></b>' +
						...
						'<img class="aliRemoveBtn" src="http://culturecloud.img-cn-hangzhou.aliyuncs.com/H5/201733161236Bn6gTqiCZBHhgObFvsCkLFNzmIBoh2.png" style="position:absolute;right:0px;top:0;width:20px;z-index:10;" />' +
						'</div>');
			...
			FileUploaded: function(up, file, info) {
				if(info.status == 200) { //回调函数
					...
					//					img.setAttribute("style", "max-height: 80px;max-width: 80px;position: absolute;left: 0;right: 0;top: 0;bottom: 0;margin: auto;");
