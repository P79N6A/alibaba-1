...
$(function(){
    ...
    $(".room-time-set input[type=text]").on("blur",function(){
        ...
        if(timePeriod.trim() != "" && timePeriod.trim() != "时间段5设置"){
            if(!timePeriodReg.test($.trim(timePeriod))){
                ...
                if (!validateTime(timePeriod)) {
                    ...
                    for(var i = (currIndex-1); i > 0; i--){
                        ...
                        if(prevTimeVal != "" &&  prevTimeVal != "时间段5设置"){
                    ...
                    for(var i = (currIndex+1); i <= totalTdLen; i++){
                        ...
                        if(nextTimeVal != "" &&  nextTimeVal != "时间段5设置"){
...
function checkTimePeriod(){
    ...
    $(".room-time-set input[type=text]").each(function(){
        var timePeriod = $(this).val();
        var timePeriodReg = /^\d{2}:\d{2}-\d{2}:\d{2}$/;
        if(timePeriod.trim() != "" &&  timePeriod.trim() != "时间段5设置" && !timePeriodReg.test(timePeriod.trim())){
            $(this).focus();
            checkResult = false;
            return checkResult;
        }
        else if(timePeriod.trim() != "" && timePeriod.trim() != "时间段5设置"){ 
        	var checkbox=$(this).parent().parent().find(":checkbox");
        	var checked=checkbox.prop("checked");
        	if(checked){
        		var id=checkbox.attr("id")
        		if($.inArray(id,checkArr)<0){
	           	 checkArr.push(id);
        		}
        	}
        }
    });
    $(".room-time-set :checkbox").each(function(){
...
function getRoomDayData(trId){
    ...
    $("#"+ trId +" td").each(function(index,val){
        ...
        if(index>0){
            ...
            if(period.trim() == "" || period.trim() == "时间段5设置") {
                //OFF代表时间段关闭
                roomDayStr = roomDayStr + "OFF" + ",";
            }else{
                roomDayStr = roomDayStr + period + ",";
            }
        }
    });
