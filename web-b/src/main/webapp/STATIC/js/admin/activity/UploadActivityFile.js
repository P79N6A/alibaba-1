...
$(function () {
	...
	$("#file2").uploadify({
		...
		fileTypeExts:'*.doc;*.docx;*.xls;*.xlsx;*.txt;*.pdf', //控制可上传文件的扩展名，启用本项时需同时声明fileDesc
		'auto':true, //true当选择文件后就直接上传了，为false需要点击上传按钮才上传
		...
		onUploadSuccess:function (file, data, response) {
			var json = $.parseJSON(data);
			var url=json.data;
			var attachmentUrl = $("#activityAttachment").val();
			if("" ==attachmentUrl){
				$("#activityAttachment").val(url);
			}else{
				$("#activityAttachment").val(attachmentUrl+","+url);
			}
