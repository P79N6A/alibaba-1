...
public class CmsOrderController {
    ...
    public String updateOrderByActivityOrderId(String activityOrderId,String[] cancelSeat) {
        ...
        if (user != null) {
            ...
            Map<String,String> map = cmsActivityOrderService.cancelUserOrder(activityOrderId,null,orderLines,null);
    ...
    @ResponseBody
    public String refundSuccess(String activityOrderId) {
         ...
         if (user != null) {
 			...
 			cmsActivityOrder.setActivityOrderId(activityOrderId);
 			cmsActivityOrder.setOrderUpdateTime(new Date());
 			// 退款成功
 			cmsActivityOrder.setOrderPaymentStatus((short)4);
 			cmsActivityOrder.setOrderRefundTime(new Date());
    ...
    public void exportOrderExcel(HttpServletResponse response,String searchKey,String activityIsFree,String activityArea,String orderVotes,String orderPayStatus,String orderPaymentStatus,String activitySalesOnline,
    		String activityOrderStartTime,String activityOrderEndTime) throws Exception {
        ...
        if(user != null){
            ...
            if(StringUtils.isNotBlank(orderPaymentStatus)){
            	 activity.setOrderPaymentStatus(Short.parseShort(orderPaymentStatus));
            }
            if(StringUtils.isNotBlank(activityIsFree)){
            	activity.setActivityIsFree(Integer.parseInt(activityIsFree));
            }
            
            activity.setActivitySalesOnline(activitySalesOnline);
            //添加时间筛选
            activity.setActivityOrderStartTime(activityOrderStartTime);
            activity.setActivityOrderEndTime(activityOrderEndTime);
            ...
            for (CmsActivity cmsActivity : activityList) {
                String orderSummary = "";
                if(StringUtils.isNotBlank(cmsActivity.getOrderSummary())){
                    String[] seatVal = cmsActivity.getOrderSummary().split(",");
                    for (int i = 0; i < seatVal.length; i++) {
                        orderSummary+=seatVal[i].split("_")[0] + "排" + seatVal[i].split("_")[1] + "座,";
                    }
                }else{
                    for (int i = 1; i <= cmsActivity.getOrderVotes(); i++) {
                        orderSummary+= "票"+i+",";
                    }
                }
                cmsActivity.setOrderSummary(orderSummary.substring(0,orderSummary.length()-1));
            }
