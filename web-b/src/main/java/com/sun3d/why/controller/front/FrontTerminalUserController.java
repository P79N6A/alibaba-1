...
public class FrontTerminalUserController {
    ...
    public void terminalLogin(CmsTerminalUser user,String rememberUser, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String loginErr ="0";
            try {
                loginErr = cacheService.getValueByKey(CacheConstant.LOGIN_ERROR+new SimpleDateFormat("yyyy-MM-dd").format(new Date())+user.getUserName());
            }catch (Exception e){
                e.printStackTrace();
            }
            if(StringUtils.isNotBlank(loginErr)&&Integer.valueOf(loginErr)>10){
                jsonObject.put("status","LoginLimit");
...
/*            if(StringUtils.isEmpty(user.getUserMobileNo())&&StringUtils.isEmpty(user.getUserTelephone())){
            ...
            if(StringUtils.isNotBlank(tuser.getUserMobileNo())){
                if (user.getUserMobileNo().equals(tuser.getUserMobileNo())){
                    ...
                    if (Constant.RESULT_STR_SUCCESS.equals(terminalUserService.editTerminalUserById(user))) {
                        ...
                        finalCmsTerminalUser.setSourceCode(sessionUser.getSourceCode());
                        finalCmsTerminalUser.setSysId(sessionUser.getSysId());
    ...
    @ResponseBody
    public Map<String,Object> sendCode(String userMobileNo,String userName,String userPwd,Integer userSex) {
            try {
                return terminalUserService.sendSmsCode(userMobileNo,userName,userPwd,userSex);
            } catch (Exception e) {
...
//    /*************该方法已废弃******************/
    ...
    @ResponseBody
    public String valForgetCode(String code){
    ...
    public String setNewPass(CmsTerminalUser user,String reqCode){
        ...
        try{
            ...
            final CmsTerminalUser finalCmsTerminalUser = user;
            if(extUser != null) {
                finalCmsTerminalUser.setSourceCode(extUser.getSourceCode());
                finalCmsTerminalUser.setSysId(extUser.getSysId());
            }
            Runnable runnable = new Runnable() {
                @Override
                public void run() {
