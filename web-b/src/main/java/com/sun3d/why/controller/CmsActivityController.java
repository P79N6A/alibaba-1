...
public class CmsActivityController {
    ...
    @ResponseBody
    public Object addActivity(final CmsActivity activity) {
        try {
        	 System.out.println("OrderPrice============================="+activity.getActivityPayPrice());
            if (activity != null) {
...
/*            if(cmsActivitySeat.getSeatIsSold() != null && cmsActivitySeat.getSeatIsSold() == 2){
    ...
    public String toEditRatingsInfo(String activityId, String type, HttpServletRequest request) {
        if (StringUtils.isNotEmpty(activityId)) {
            ...
            } else if (StringUtils.isNotBlank(type) && Constant.GATHER.equals(type)) {
                ratingsInfo = cmsGatherService.queryGatherByPrimaryKey(activityId).getGatherGrade();
            }
        ...
        if (StringUtils.isNotEmpty(activityId) && StringUtils.isNotEmpty(ratingsInfo)) {
            try {
                ...
                if (StringUtils.isNotBlank(type) && Constant.EDITORIAL.equals(type)) {
                	...
                	CmsGather cmsGather = new CmsGather();
                	cmsGather.setGatherId(activityId);
                	cmsGather.setGatherGrade(ratingsInfo);
                    status = cmsGatherService.saveOrUpdateGather(cmsGather).equals("200")?true:false;
                }
                if (status) {
                    return Constant.RESULT_STR_SUCCESS;
                }
