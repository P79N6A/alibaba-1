...
//                map.put("deptParentId",venue.getVenueParentDeptId());
     ...
     * @return
     */
    @RequestMapping("/relationVenueIndex")
    public ModelAndView relationVenueIndex(CmsVenue venue,String memberId,Integer relationType, Pagination page) {
        ModelAndView model = new ModelAndView();
        try {
            SysUser user = (SysUser) session.getAttribute("user");
            String userDeptPath = null;
            ...
            venue.setVenueState(Constant.PUBLISH);
            List<CmsVenue> list = cmsVenueService.relationVenueByCondition(venue,memberId,relationType,page, userDeptPath);
            model.addObject("venueList", list);
            model.addObject("page", page);
            model.addObject("venue", venue);
            model.addObject("memberId", memberId);
            model.setViewName("admin/league/relationVenueIndex");
        } catch (Exception e) {
