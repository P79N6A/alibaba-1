...
//            }
     ...
     */
    @Override
    public String webLogin(CmsTerminalUser user) {
        try {//只查询一次
            String result = addTerminalUser(user);
            CmsTerminalUser terminalUser = new CmsTerminalUser();
            if (Constant.RESULT_STR_MOBILE_REPEAT.equals(result)) {
                terminalUser = cmsTerminalUserMapper.getCmsTerminalUserByMobile(user.getUserMobileNo());
                if(!terminalUser.getUserId().equals(MD5Util.toMd5(user.getUserName()))){
                    return Constant.RESULT_STR_FAILURE;
                }
            } else {
                terminalUser = cmsTerminalUserMapper.queryTerminalUserById(MD5Util.toMd5(user.getUserName()));
            }
    ...
    public String addTerminalUser(CmsTerminalUser user) {
        try {
            if (user != null) {
                ...
                user.setUserId(MD5Util.toMd5(user.getUserName()));
