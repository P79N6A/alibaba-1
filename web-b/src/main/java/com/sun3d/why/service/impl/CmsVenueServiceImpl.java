...
public class CmsVenueServiceImpl implements CmsVenueService {
    ...
    public List<AreaData> queryVenueAllType(String venueArea) {
        ...
        if (venueArea.equals("4300")){
            sqlMap.put("venueArea",",%");
        }else {
...
//        }
    ...
    public List<CmsVenue> queryVenueByCondition(CmsVenue venue, Pagination page, PaginationApp pageApp, String userDeptPath) {
        ...
        if(StringUtils.isNotBlank(venue.getVenueArea())){
            if (venue.getVenueArea().equals("4300")){
                map.put("venueArea",",市级自建");
            }else {
...
//            SysUser syncUser = cmsUserService.querySysUserByUserAccount("chuangtu_sh");
           venue.setVenueCreateUser(sysUser.getUserId());
  ...
  //          venue.setVenueDept(syncUser.getUserDeptPath());
     ...
     */
    @Override
    public List<CmsVenue> relationVenueByCondition(CmsVenue venue,String memberId,Integer relationType,
                                                   Pagination page, String userDeptPath) {
        Map<String, Object> map = new HashMap<String, Object>();
        ...
        //网页分页
        if (page != null && page.getFirstResult() != null && page.getRows() != null) {
            int total = cmsVenueMapper.queryRelateVenueByCondition(map).size();
            page.setTotal(total);
            map.put("firstResult", page.getFirstResult());
            map.put("rows", page.getRows());
        }
        return cmsVenueMapper.queryRelateVenueByCondition(map);
    }

	@Override
	public List<CmsVenue> queryVenueByConditionFromCulturalOrder(CmsVenue venue) {
		return cmsVenueMapper.queryVenueByConditionFromCulturalOrder(venue);
	}
