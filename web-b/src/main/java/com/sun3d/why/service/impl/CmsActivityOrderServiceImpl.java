...
/*                        this.cancelApiOrder(activityOrder,cmsActivity);*/
    ...
    public Map cancelUserOrder(String activityOderId, CmsTerminalUser cmsTerminalUser, String orderSeat, Integer cancelCount) {
        Map map = new HashMap();
        int cancelResult = 0;
        final CmsActivityOrder cmsActivityOrder = queryCmsActivityOrderById(activityOderId);
        ...
        if (cmsActivityOrder != null) {
            ...
            if (orderSeat != null && StringUtils.isNotBlank(orderSeat)) {
                ...
                for (String orderLine : seatIds) {
                    ...
                    activityOrderDetail.setSeatStatus(1);
    ...
    public void revocationActivitySendSMS(String activityId, String msgSMS, String userId) {
        ...
        if (cmsActivityOrders != null && cmsActivityOrders.size() > 0) {
            for (CmsActivityOrder cmsActivityOrder : cmsActivityOrders) {
                Map<String,String> result = cancelUserOrder(cmsActivityOrder.getActivityOrderId(),null,null,null);
                Map map = new HashMap();
