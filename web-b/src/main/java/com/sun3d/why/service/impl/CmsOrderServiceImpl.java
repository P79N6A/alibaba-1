...
public class CmsOrderServiceImpl implements CmsOrderService {
    ...
    public List<CmsActivity> queryCmsOrderByCondition(CmsActivity activity, Pagination page, SysUser sysUser) throws Exception{
        ...
        // 支付状态
        if(activity != null && activity.getOrderPaymentStatus() != null ){
            map.put("orderPaymentStatus",activity.getOrderPaymentStatus());
        }
        ...
        //添加时间筛选
        if(activity != null && StringUtils.isNotBlank(activity.getActivityOrderStartTime())){
        	try {
                map.put("activityOrderStartTime",df.parse(activity.getActivityOrderStartTime() + " 00:00:00"));
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
        if(activity != null && StringUtils.isNotBlank(activity.getActivityOrderEndTime())){
        	try {
                map.put("activityOrderEndTime",df.parse(activity.getActivityOrderEndTime() + " 23:59:59"));
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
