...
public class CmsCommentServiceImpl implements CmsCommentService {
    ...
    public boolean deleteCommentById(final String commentId) {
        try{
        	final CmsComment comment=commentMapper.queryCommentById(commentId);
            ...
            if(r>0){
            	Runnable runnable = new Runnable() {
                    ...
                    public void run() {
        				...
        				userIntegralDetailService.deteleIntegralByVo(userIntegralDetail);
                    }
            	};
    ...
    public List<CmsComment> queryCommentByCondition(CmsComment comment, Pagination page, SysUser sysUser) {
        ...
        if(comment != null){
            ...
            if(comment.getCommentState() != null){
                map.put("commentState", comment.getCommentState());
            }
        }
        //权限验证
            ...
            for(CmsComment cmsComment:commentList){
                if(cmsComment.getCommentType() != null){
                    if(cmsComment.getCommentType() == Constant.TYPE_VENUE){
                    	...
                    	CcpNyImg ccpNyImg = ccpNyImgMapper.selectByPrimaryKey(cmsComment.getCommentRkId());
                    	CcpWalkImg ccpWalkImg = ccpWalkImgMapper.selectByPrimaryKey(cmsComment.getCommentRkId());
                        ...
                        }else if(ccpNyImg != null && StringUtils.isNotBlank(ccpNyImg.getNyImgUrl())){		//文化新年
                            cmsComment.setCommentRkName(ccpNyImg.getNyImgUrl());
                        }else if(ccpWalkImg != null && StringUtils.isNotBlank(ccpWalkImg.getWalkImgUrl())){		//行走故事
                            cmsComment.setCommentRkName(ccpWalkImg.getWalkImgUrl());
                        }
                    ...
                    }else if(cmsComment.getCommentType() == Constant.TYPE_CULTURE_SQUARE){
                    	CmsCulturalSquare cmsCulturalSquare = cmsCulturalSquareMapper.selectByPrimaryKey(cmsComment.getCommentRkId());
                        if(cmsCulturalSquare != null && StringUtils.isNotBlank(cmsCulturalSquare.getExt1())){
                            cmsComment.setCommentRkName(cmsCulturalSquare.getExt1());
                        }
                    }else if(cmsComment.getCommentType() == Constant.TYPE_CULTURE_LIVE){
                    	CcpLiveActivity ccpLiveActivity = ccpLiveActivityMapper.selectByPrimaryKey(cmsComment.getCommentRkId());
                        if(ccpLiveActivity != null && StringUtils.isNotBlank(ccpLiveActivity.getLiveTitle())){
                            cmsComment.setCommentRkName(ccpLiveActivity.getLiveTitle());
                        }
                    }else if(cmsComment.getCommentType() == Constant.TYPE_BPINFO){
                    	BpInfo bpInfo = bpInfoMapper.selectByPrimaryKey(cmsComment.getCommentRkId());
                        if(bpInfo != null && StringUtils.isNotBlank(bpInfo.getBeipiaoinfoTitle())){
                            cmsComment.setCommentRkName(bpInfo.getBeipiaoinfoTitle());
                        }
                    }else if(cmsComment.getCommentType() == Constant.TYPE_BPANTIQUE){
                    	BpAntique bpAntique = bpAntiqueMapper.queryBpAntiqueById(cmsComment.getCommentRkId());
                        if(bpAntique != null && StringUtils.isNotBlank(bpAntique.getAntiqueName())){
                            cmsComment.setCommentRkName(bpAntique.getAntiqueName());
                        }
                    }else if(cmsComment.getCommentType() == Constant.TYPE_BPPRODUCT){
                    	BpProduct bpProduct = bpProductMapper.queryBpProductById(cmsComment.getCommentRkId());
                        if(bpProduct != null && StringUtils.isNotBlank(bpProduct.getProductName())){
                            cmsComment.setCommentRkName(bpProduct.getProductName());
                        }
                    }else if(cmsComment.getCommentType() == 30){
                    	CmsCulturalOrder order = cmsCulturalOrderService.queryCulturalOrderById(cmsComment.getCommentRkId());
                    	if (order != null && StringUtils.isNotBlank(order.getCulturalOrderName())){
                    		cmsComment.setCommentRkName(order.getCulturalOrderName());
                    	}
                    }
                }
            }
    ...
    public List<CmsComment> queryCmsCommentByCondition(CmsComment comment, Pagination page){
        ...
        if(comment.getCommentType() == null || comment.getCommentType() == 0) { //这个值没有传进来，默认取场馆。修改于19.2.19，因调试培训模块评论时发现；不确定对其他模块是否有影响
     ...
     */
    @Override
    public int checkCommentState(CmsComment comment){
        return commentMapper.checkCommentState(comment);
    }
    ...
    public CmsComment queryCommentById(String commentId){
        ...
        if(comment != null){
            ...
            }else if(comment.getCommentType() == Constant.TYPE_BPINFO){
	            BpInfo bpInfo = bpInfoMapper.selectByPrimaryKey(comment.getCommentRkId());
	            if(bpInfo != null && StringUtils.isNotBlank(bpInfo.getBeipiaoinfoTitle())){
	                comment.setCommentRkName(bpInfo.getBeipiaoinfoTitle());
	            }
            } else if(comment.getCommentType() == 30){
            	CmsCulturalOrder order = cmsCulturalOrderService.queryCulturalOrderById(comment.getCommentRkId());
            	if (order != null && StringUtils.isNotBlank(order.getCulturalOrderName())){
            		comment.setCommentRkName(order.getCulturalOrderName());
            	}
            }
        }
        return comment;
