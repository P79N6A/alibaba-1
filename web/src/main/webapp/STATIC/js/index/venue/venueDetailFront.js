...
/*function roomBook(roomId){
    dialogAlert("提示", "请前往大连文化云微信公众号预订!");
    return;
    if($("#isLogin").val() == 1){
        ...
        function(data) {
            ...
            if(data != null && data != ""){
                ...
                for(var i=0; i<roomList.length; i++){
                    ...
                    "<h4><a href='javascript:;' onclick=roomBook('"+roomId+"')>"+roomName+"</a></h4>"+
                    "<p>面积："+roomArea+"平方米</p>"+
                    "<p>可容纳人数："+roomCapacity+"人</p>";
                    if(sysNo == 0 || sysNo == undefined || sysNo.trim()==""){
                        if(availableCount > 0){
                            roomHtml = roomHtml + "<a href='javascript:;' class='book-room' onclick=roomDetail('"+roomId+"')>可预订</a>";
                        }
                    }
...
function loadActivityList(){
    var venueId = $("#venueId").val();
    var data = {"venueId":venueId};
    $.post("../frontVenue/activityList.do",data,
        function(data) {
            var activityHtml = "";
            if(data != null && data != ""){
                var activityCount = data.activityCount;
                if(activityCount>relatedActivityCount){
                    $("#activityViewMore").css("display","");
                }
                var activityList = data.activityList;
                //显示活动室信息
                for(var i=0; i<activityList.length; i++){
                    var activity = activityList[i];
                    var activityIconUrl = activity.activityIconUrl;
                    var activityName = activity.activityName;
                    var activityId = activity.activityId;
                    var activityTime = activity.activityStartTime;
                    var activityAddress = activity.activityAddress;
                    activityHtml = activityHtml + "<li data-id='"+activityIconUrl+"'>"+
                    "<a href='javascript:;' class='room_pic fl' onclick=activityDetail('"+activityId+"')><img src='../STATIC/image/venues-img2.jpg' alt='' width='135' height='100'/></a>"+
                    "<div class='room_txt fl'> " +
                    "<h4><a href='javascript:;' onclick=activityDetail('"+activityId+"')>"+activityName+"</a></h4>"+
                    "<p>时间："+activityTime+"</p>"+
                    "<p>地点："+activityAddress+"人</p>";
                    activityHtml = activityHtml + "</div></li>";
                }
            }
            if(activityHtml == ""){
                $("#activity-list-div").css("display","none");
            }else{
                $("#activity-list-div ul").html(activityHtml);
            }
            loadActivityListPics();
        });
}
...
//获取列表元素中所包含的图片
function loadActivityListPics(){
    //请求页面下方团体所有图片
    $("#activity-list-div li").each(function (index, item) {
        var imgUrl = $(this).attr("data-id");
        if(imgUrl == undefined || imgUrl == null || imgUrl == ""){
            $(item).find("img").attr("src", "../STATIC/image/default.jpg");
        }else{
        ...
        function(data) {
            if(data != null && data != ""){
                ...
                for(var i=0; i<roomList.length; i++){
                    ...
                    var availableCount = room.availableCount;
                    roomHtml = roomHtml + "<li   data-id='"+roomPicUrl+"'>"+
                    "<a href='javascript:;' class='room_pic fl' onclick=roomBook('"+roomId+"')><img src='../STATIC/image/room-img1.jpg' alt='' width='135' height='100'/></a>"+
                    "<div class='room_txt fl'> " +
                    "<h4><a href='javascript:;' onclick=roomBook('"+roomId+"')>"+roomName+"</a></h4>"+
                    "<p>面积："+roomArea+"平方米</p>"+
                    "<p>可容纳人数："+roomCapacity+"人</p>";
...
function loadMoreActivity(){
    var venueId = $("#venueId").val();
    var pageNum = parseInt($("#activityPageNum").val())+1;
    $("#activityPageNum").val(pageNum);
    var data = {"venueId":venueId,"pageNum":pageNum};
    $.post("../frontVenue/activityList.do",data ,
    		function(data) {
	        var activityHtml = "";
        ...
        $("#activity-list-div ul").append(activityHtml);
        if(activityCount <= pageNum*relatedActivityCount){
            $("#activityViewMore").css("display","none");
        }
        loadActivityListPics();
    });
}
