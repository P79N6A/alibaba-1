...
function doBook() {
    ...
    if($("#orderPhoneNo").val() != undefined && $("#orderPhoneNo").val() == ''){
        dialogAlert("提示", "请输入预订电话号码!");
        return;
    }
    var reg = /^1\d{10}$/;
    if(!reg.test($("#orderPhoneNo").val())){
        dialogAlert("提示", "请输入正确的预订电话号码!");
        return;
    }
    ...
    if($("#bookCount").val() != undefined && $("#bookCount").val() == ''){
        dialogAlert("提示", "请输入预订数量!");
        return;
    }
    if($("#bookCount").val() != undefined) {
        var reg = new RegExp("^[0-9]*$");
        if(!reg.test($("#bookCount").val()) || $("#bookCount").val() == 0){
            dialogAlert("提示", "请输入正确的预订数量!");
            return;
        }
    }
/*    if(!valCode()) {
        dialogAlert("提示", "请输入正确的验证码!");
        return;
    }*/
    ...
    $.ajax({
        ...
        url: "../userTicket/checkFrontActivityBook.do",//请求的action路径
