...
function doBook() {
    ...
    if(!reg.test($("#orderPhoneNo").val())){
        removeMsg("orderPhoneNoMsg");
        appendMsg("orderPhoneNoMsg","请输入正确的预订电话号码!");
        $('#orderPhoneNo').focus();
        return;
    } else {
        removeMsg("orderPhoneNoMsg");
...
/*    if(!valCode()) {
    ...
    $.ajax({
        ...
        url: "../ticketActivity/checkTicketActivityBook.do",//请求的action路径
        ...
        success:function(data){ //请求成功后处理函数。
            if(data.success == 'Y') {
                ...
                if (data.msg == 'activityEmpty') {
                    ...
                    dialogAlert("提示", "超过最大订票数");
                } else if (data.msg == 'moreLimitCount') {
                    dialogAlert("提示", "超过单笔最大购票数");
                } else {
...
function showSeatInfo() {
    ...
    $("#eventDateTime").val(eventDateTime);
    $.post("../ticketActivity/showTicketActivitySeatInfo.do", $("#bookForm").serialize(),
...
function queryEventTime() {
    ...
    $.post("../ticketActivity/queryTicketCanBookEventTime.do", formData,
        function(map){ //请求成功后处理函数。
            var list = eval(map);
            var innerHtml = "";
            var firstEventTime = ""
