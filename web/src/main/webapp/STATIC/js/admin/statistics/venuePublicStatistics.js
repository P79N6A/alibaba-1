...
app.controller('alteCon', function ($scope, $http) {
    ...
    $scope.$watch('area.venue', function (data) {
        if (data == '0' || data == '') {
            if($scope.type=='venue' && $scope.startTime==''){
                var date = new Date();
                var seperator1 = "-";
                var month = date.getMonth() + 1;
                var day = date.getDate() -1;
                $scope.startTime= date.getFullYear()+seperator1+month+seperator1+day;
            }
            $scope.selMod = true;
            $scope.dateType = '';
        } else {
    ...
    $scope.getData = function () {
        ...
        if ($scope.area.district) {
            $http.get('../activityStatistics/selectStatDataByAdmin.do', {
                    params: {
