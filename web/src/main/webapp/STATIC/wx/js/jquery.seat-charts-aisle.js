...
(function($) {
	...
	$.fn.seatCharts = function (setup) {
		...
		//if (this.data('seatCharts')) { //如果要显示多个座位编辑器，删除这段代码
				...
				return function (setup) {
					...
					fn.settings.$node
						.attr({
							...
							//colname        : (currentRow+1)+"排"+fn.settings.label+"座"
						})
						///.text(fn.settings.label)  //座位号
						.text("")
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left) {
				/*$row.append(
					$('<div></div>')
						...
						.text("第"+settings.naming.rows[row]+"排").css({"width": 50, "margin": 5})
				);*/
				$("#seat-tit").append(
					$('<div></div>')
						.addClass('seatCharts-cell seatCharts-space')
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) {
				...
				$row.append(character != '_' ?
						...
						(function(naming) {
							...
							seats[id] = new seat({
								...
								column    : column,
								character : character,
								colname   : (currentRow+1)+"排"+(overrideLabel ? overrideLabel : naming.getLabel(character, naming.rows[row], naming.columns[column]))+"座"
							});
							seatIds.push(id);
				...
				character != '_'? delNum : delNum++;
			});
			if(delNum == settings.columnNum){
				$row.find(".seatCharts-cell:first-child").text("");
			}else{
				currentRow++;
				//$row.find(".seatCharts-cell:first-child").text("第"+currentRow+"排");
				//$row.find(".seatCharts-cell:first-child").text(currentRow);
			}
			fn.append($row);
		});
