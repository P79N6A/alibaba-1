jQuery(function() {
	...
	uploader.on('uploadSuccess', function(file, response) {		
	    ...
	    if(response.status==0){
    		...
    		$(".upImg").attr("src",imgUrlAli+"@200w");
    		
    		var shareSuccessImg=$("#shareSuccessImg").val();
    		var shareHelpImg=$("#shareHelpImg").val();
    		
    		var data={userId:userId}
    		
    		if(shareSuccessImg)
    			data.shareSuccessImg=imgUrlAli;
    		else if(shareHelpImg)
    			data.shareHelpImg=imgUrlAli;
    			
    		$.post("../wechatRedStar/saveUserInfo.do",data,function(result){
