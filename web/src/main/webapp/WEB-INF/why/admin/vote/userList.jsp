...
<div class="search">
	...
	<div class="search-box">
        <i></i><input value="<c:choose><c:when test="${not empty voteUser.userName}">${voteUser.userName}</c:when><c:otherwise>请输入用户名称或手机号</c:otherwise></c:choose>" name="userName" id="userName" class="input-text" data-val="请输入用户名称或手机号" type="text"/>
    </div>
...
<div class="main-content">
   <table width="100%">
        ...
        <c:if test="${!empty userList}">
            <c:forEach items="${userList}" var="userMessage" varStatus="status">
            <%i++; %>
                <tr>
                  <td width="170"><%=i %></td>
