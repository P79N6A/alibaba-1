...
<form id="inheritorForm">
    <div class="main-publish tag-add">
        ...
        <input type="hidden" name="inheritorId" value="${inheritor.inheritorId}"/>
        <input type="hidden" name="cultureId" value="${inheritor.cultureId}"/>
            <table width="100%" class="form-table">
                ...
                <tr>
                    ...
                    <td class="td-input" id="inheritorNameLabel"><input type="text" class="input-text w210" id="inheritorName" name="inheritorName" maxlength="20" value="${inheritor.inheritorName}"/></td>
                </tr>
                <tr>
                    <td class="td-title">性别：</td>
                    <td class="td-radio">
                        <label><input type="radio" value="1" name="inheritorSex" <c:if test="${inheritor.inheritorSex eq 1}">checked</c:if>/><em>男</em></label>
                        <label><input type="radio" value="2" name="inheritorSex" <c:if test="${inheritor.inheritorSex eq 2}">checked</c:if>/><em>女</em></label>
                    </td>
                </tr>
                <tr>
                    <td class="td-title">年龄：</td>
                    <td class="td-input" id="inheritorAgeLabel"><input type="text" id="inheritorAge" class="input-text w210" name="inheritorAge" onkeyup="this.value=this.value.replace(/\D/g,'')" maxlength="3" value="${inheritor.inheritorAge}"/></td>
                </tr>
                <tr>
                    ...
                    <td class="td-select" id="inheritorNationLabel">
                        <input type="hidden" name="inheritorNation"  id="inheritorNation" value="${inheritor.inheritorNation}"/>
                        ...
                        <table>
                            <tr>
                                <td id="inheritorHeadImgUrlLabel">
                                    <input type="hidden"  name="inheritorHeadImgUrl" id="inheritorHeadImgUrl" value="${inheritor.inheritorHeadImgUrl}">
                ...
                <tr class="td-line">
                    ...
                    <td class="td-input">
                        <textarea rows="4" name="inheritorRemark" class="textareaBox" style="width: 500px;resize: none">${inheritor.inheritorRemark}</textarea>
                    </td>
                </tr>
                <tr>
...
<script type="text/javascript">
    $(function(){
        showNationSelectData("inheritorNationType", "${path}/sysdict/queryCode.do?dictCode=NATION_TYPE", "请选择民族", $("#inheritorNation").val());
    ...
    seajs.use(['jquery'], function ($) {
        $(function () {
            ...
            $(".btn-save").on("click", function(){
                ...
                $.post("${path}/inheritor/editInheritor.do", $("#inheritorForm").serialize(), function(result) {
                    if (result == "success") {
