                ...
                <tr>
                    ...
                    <td>
                        ...
                        <c:if test="${not empty avct.activityState && avct.activityState ==5}">
                            <%if (activityEditorialReturnRecycleButton) {%>
                            <a target="main" href="javascript:returnBack('${avct.activityId}')">还原</a>|
                            <%}%>
                            <a class="copyButton"
                               data-clipboard-text='${avct.activityUrl}'
                               href="javascript:copyUrl('${avct.activityUrl}');">复制URL</a>
                        </c:if>
                            <%--采编草稿箱--%>
                        <c:if test="${not empty avct.activityState &&  avct.activityState ==1}">
                            <%if (addActivityEditorial) {%>
                            <a target="main"
                               href="${path}/activityEditorial/preEditActivityEditorial.do?id=${avct.activityId}">编辑</a> |
                            <%}%>
                            <%if (activityEditorialPublishButton) {%>
                            <a href="javascript:;" onclick="publishActivity('${avct.activityId}')">发布</a> |
                            <%}%>
                            <a class="copyButton"
                               data-clipboard-text='${avct.activityUrl}'
                               href="javascript:copyUrl('${avct.activityUrl}');">复制URL</a> |
                            <%if (activityEditorialDeleteButton) {%>
                            <a target="main" href="javascript:delActivity('${avct.activityId}');">删除</a>
                            <%}%>
                        </c:if>
                            <%--采编列表--%>
                        <c:if test="${not empty avct.activityState && avct.activityState==6}">
                            ...
                            <%}%>
                            <c:if test="${not empty avct.ratingsInfo}">
                                <%if (activityEditorialEditRating) {%>
                                <a onclick="toEditRatingsInfo('${avct.activityId}')">修改评级</a>
                                <%}%>
                            </c:if>
                            <c:if test="${ empty avct.ratingsInfo}">
                                <%if (activityEditorialRating) {%>
