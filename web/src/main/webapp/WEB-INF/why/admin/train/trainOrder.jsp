...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('.pxOrderList').on('click', '.pic,.tit', function () {
            window.location.href='${path}/wechatTrain/trainDetail.do?id='+ $(this).attr('v-id');
        });
        ...
        $('.pxPopBox .pxpBtn').on('click', '.ok', function () {
            ...
            $.ajax({
                ...
                async: false,
                data: {id:orderId, state: 2,trainTitle:trainTitle,name:orderName,phoneNum:phoneNum},
    ...
    function loadOrderList() {
        ...
        $.ajax({
            ...
            async: false,
            data: {page: page, queryType: $("#queryType").val()},
            error: function () {//请求失败处理函数
                alert('请求失败');
            },
            success: function (data) { //请求成功后处理函数。
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '</div><div class="char">\n' +
                        ...
                        '                <div class="wz">开课时间：' + obj.trainStartTime.substr(0, 10).replace(/-/g, '.') + '-' + obj.trainEndTime.substr(0, 10).replace(/-/g, '.') + '</div>\n' +
                        '                <div class="wz">上课地址：' + obj.trainAddress + '</div>\n';

                    html += '                <div class="tuid clearfix">\n';
                    if (queryType==1 && (obj.state == 1 || obj.state == 3)) {// 1:录取通过，2：退订，3：审核中，4：审核未通过
