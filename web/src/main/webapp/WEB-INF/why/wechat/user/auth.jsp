...
<script>
	...
	function next()
	{
		if (userId ==null || userId == '') {
    		...
    		var userAuthType = $("#userAuthType").val();
    		...
    		}else if(userAuthType == 2){
            	//港澳居民通行证
               	var greg = /^[HMhm]{1}([0-9]{10}|[0-9]{8})$/;
               	if(!greg.test(idCardNo)){
                    dialogAlert('提示', "证件号不合法!");
                    return  false;
                }
            }else if(userAuthType == 3){
            	//台湾居民通行证
          		var treg = /^[0-9]{8}$/;
          		if(!treg.test(idCardNo)){
                    dialogAlert('提示', "证件号不合法!");
                    return  false;
                }
    		}
			...
			<div class="content padding-bottom0 margin-bottom100">
				...
				<div class="certification bgfff fs30 margin-top20 padding-top20">
					<div class="border-bottom padding-bottom20">
						...
						<select id="userAuthType" class="f-right selTag" onchange="selectAuthType(this.value)"<%--  ${userType==2?'disabled':""} --%>
						style="float:left!important;border: none;margin-top: 5px;">
						  <option value ="0" <%-- ${user.userAuthType==0?'selected':""} --%>>身份证</option>
						  <%-- <option value ="1" ${user.userAuthType==1?'selected':""}>护照</option> --%>
						  <option value="2" <%-- ${user.userAuthType==2?'selected':""} --%>>港澳居民通行证</option>
						  <option value="3" <%-- ${user.userAuthType==3?'selected':""} --%>>台湾居民通行证</option>
						</select>
						<c:choose>
							<c:when test="${userType!=2 }">
							<input placeholder="请输入证件号码" style="border: none;margin-top: 5px;" class="f-left c808080 w480" type="text" name="userCardNo" id="userCardNo" value="${user.userCardNo }" />
							</c:when>
							<c:otherwise>
