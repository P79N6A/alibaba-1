...
<head>
    ...
    <style>
       body {
			...
			.list_block .msZhanshi li {
				margin: 0 30px;
				border-top: 1px solid #f5f9fb;
				padding: 32px 0;
			}
			.list_block .msZhanshi li:first-child {
				border-top: 0;
			}
			.list_block .msZhanshi .tit {
				font-size: 32px;
				color: #333333;
				line-height: 1.5;
				margin-bottom: 10px;
			}
			.list_block .msZhanshi .ner {
				font-size: 28px;
				color: #999999;
				line-height: 40px;
				height: 120px;
...
<div class="common_container">
    ...
    <div class="list_block" id="orderCustomInfoDiv" style="display: none;"><ul class="msZhanshi"></ul></div>
    <div class="list_block">
        ...
        $.post("${path}/wechatActivity/activityWcDetail.do", {
            ...
            if (result.status == 1) {
                ...
                if (result.data.identityCard == 1) {
                    ...
                    $("#userIdCard").html("<span class='f-left'>身份证或通行证：</span>" +
                            "<input id='order_idcard' class='fs28' type='text' style='float:left;height: 52px;border: none;' placeholder='预订人证件号' maxlength='18'></input>"
                            +"<br/><div style='font-size:28px;color:grey;'><span style='color:red;'>提示</span>:可输入身份证、港澳居民通行证或台湾居民通行证的证件号码。</div>");
                }
                //单场次
                ...
                //填写额外信息
                activityCustomInfo = result.data.activityCustomInfo;
                if(activityCustomInfo && activityCustomInfo.length>0){
                	var activityCustomInfoObj = JSON.parse(result.data.activityCustomInfo);
                	$.each(activityCustomInfoObj, function (i, dom) {
						if(dom.type == "text"){
							$("#orderCustomInfoDiv ul").append("<li>" +
																	"<div class='tit' text="+dom.title+">"+dom.title+"：</div>" +
																	"<textarea class='ner' maxlength='"+dom.desc+"'></textarea>" +
															   "</li>");
						}
					});
                	$("#orderCustomInfoDiv").show();
                }
        ...
        if (userId == null || userId == '') {
            ...
            if (identityCard == 1) {
            	...
            	var idCardReg = (/^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/);
             	//港澳居民通行证
               	var greg = /^[HMhm]{1}([0-9]{10}|[0-9]{8})$/;
              	//台湾居民通行证
          		var treg = /^[0-9]{8}$/;
          		var check = 0;
          		if(userIdCard.match(idCardReg) || userIdCard.match(greg) || userIdCard.match(treg)){
          			check = 1;
          		}
                if (userIdCard.trim().length == 0) {
                    dialogAlert('系统提示', '请输入证件号码！');
                    $("#orderSubmit").attr("onclick", "orderSubmit();");
                    return false;
                } else if (check < 1) {
                    dialogAlert('系统提示', '请正确填写证件号码！');
                    $("#orderSubmit").attr("onclick", "orderSubmit();");
                    return false;
                }else{
                	identityCardHtml = "<p>证件："+userIdCard+"</p>";
                }
            }
            ...
            if(activityCustomInfo && activityCustomInfo.length>0){
                ...
                $("#orderCustomInfoDiv ul li").each(function(){
                	if($(this).find("textarea").val().length==0){
                		dialogAlert('系统提示', '请输入'+$(this).find("div.tit").attr("text")+'！');
                        $("#orderSubmit").attr("onclick", "orderSubmit();");
                        orderCustomInfoFlag = false;
                        return false;
                	}
                	orderCustomInfoObj.push({"title":$(this).find("div.tit").attr("text"),"value":$(this).find("textarea").val()});
                })
    			...
    			button:[{
    				...
    				callback: function () {
    					if (/wenhuayun/.test(ua) && window.injs) {		//APP端
    						...
    						location.href = '${basePath}muser/login.do?type=${basePath}wechatActivity/preActivityDetail.do?activityId=${activityId}';
    					}
    				},
    				autofocus: true
    			}]
