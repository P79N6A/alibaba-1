...
<head>
	...
	<script>
		...
		$(function () {
			...
			$.post("${path}/wechatStatic/queryNyImgList.do",{nyImgId: nyImgId,userId:userId}, function (data) {
				if(data.length>0){
					var dom = data[0];
					$("#nyImgUrl").attr("src",dom.nyImgUrl+"@700w");
					$(".imgPreview img").attr("src",dom.nyImgUrl+"@700w");
					$(".userName").text(dom.userName);
					$("#nyImgContent").text(dom.nyImgContent);
		...
		function nyVote(){
			if(noControl != 1){
				if (userId == null || userId == '') {
	        		//判断登陆
	            	publicLogin("${basePath}wechatStatic/nyDetail.do?nyImgId="+nyImgId);
	        	}else{
		...
		<div class="roomcont jz700">
	        <div class="roomzpxq">
	            ...
	            <div class="tupian jz630" style="background-color: #7c7c7c;">
	                <img id="nyImgUrl" src="" style="max-height: 570px;max-width: 630px;display: block;margin: auto;position: absolute;left:0;top:0;right:0;bottom:0;" onclick="$('.imgPreview').fadeIn('fast');">
	            </div>
	            <p id="nyImgContent"></p>
	            <div class="lvvtouc">
	            ...
	            <div class="lccDoubleBtn jz630">
	                <a class="lan" href="javascript:nyVote();">投票</a>
