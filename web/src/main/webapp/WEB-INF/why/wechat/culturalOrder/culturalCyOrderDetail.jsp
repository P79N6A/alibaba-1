	          ...
	          //微信分享
	            wx.ready(function () {
	                ...
	                wx.onMenuShareAppMessage({
	                    title: "我在安康文化云报名了“${order.culturalOrderName}”，名额有限，快来参与吧！",
	                    desc: '欢迎进入安康文化云·文化点单',
			...
			<div class="dhshInfo">
				...
				<p>服务时段：
				  <c:forEach items="${events}" var="event">
				  	${event.culturalOrderEventTime}
				  </c:forEach>
				</p>				
				<p>服务地址：${order.culturalOrderAddress}</p>
				<p>参与人数限制：${order.totalTicketNum}</p>
			</div>
			<div class="sortInfo">
		...
		<div class="footer">
			<div class="active-footmenu">
				...
				<div class="footmenu-button5" id="orderButton" style="width:438px;">
					...
					<c:choose>
					   <c:when test="${ifend == '0'}">
					   	 <button type='button' class='button-dis'>已结束</button>
					   </c:when>
					   <c:otherwise> 
					   	<button type='button' onclick='preOrder();'>我要报名</button>
					   </c:otherwise>
					</c:choose>
				</div>
				<div style="clear: both;"></div>
