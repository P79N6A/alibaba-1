                                        ...
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <c:if test="${empty cmsActivity.sysId && orderLimit!=1}">
                                                    <c:choose>
                                                    	<%--需填写自定义信息活动--%>
                                                        <%-- <c:when test="${not empty cmsActivity.activityCustomInfo}">
                                                            <a class="book-btn" style="margin-right:12px;"
                                                            >请至公众号预订</a>
                                                        </c:when> --%>
                                                        <%--需要积分的活动--%>
                    ...
                    <p style="color: #F00;">
                        <c:if test="${cmsActivity.activityIsReservation == 1}">
                            <c:if test="${cmsActivity.activityIsFree == 1}">
                                温馨提示：本活动无需在安康文化云预约，具体参与方式请以主办方公布为准，您可拨打电话（<c:out value="${cmsActivity.activityTel}"
                                                                                 escapeXml="true"/>）进行咨询
                            </c:if>
                            <c:if test="${cmsActivity.activityIsFree == 2}">
                                温馨提示：本活动无需在安康文化云预约，需要收费，具体参与方式请以主办方公布为准，您可拨打电话（<c:out value="${cmsActivity.activityTel}"
                                                                                      escapeXml="true"/>）进行咨询
                            </c:if>
                        </c:if>
