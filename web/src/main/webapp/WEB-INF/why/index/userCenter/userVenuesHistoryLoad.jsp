    ...
    <ul>
        <c:forEach items="${cmsRoomOrders}" var="cmsRoomOrders">
            <li data-orderNum="${cmsRoomOrders.orderNo}">
                <div class="tit">
                    ...
                    <i class="btn-status btn-chupiao">
                        <c:choose>
                            ...
                            <c:when test="${cmsRoomOrders.bookStatus == 2 && cmsRoomOrders.checkStatus == 2}">
			        审核未通过
						    </c:when>
                ...
                <a class="btn btn-red btn-cancel-order" id="${cmsRoomOrders.roomOrderId}">删除订单</a>
                <a class="btn btn-blue btn-order-detail">订单详情</a>
            </li>
        </c:forEach>
        <c:if test="${empty cmsRoomOrders}">
            <div class="null_info">
                <h3>您还没有历史记录哦，<a href="${path}/frontVenue/venueList.do">去看看</a></h3>
            </div>
        </c:if>
    </ul>
