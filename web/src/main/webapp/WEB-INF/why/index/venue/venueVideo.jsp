...
<body>
	...
	<div class="crumb">您所在的位置：<a href="${path}/frontVenue/venueList.do">场馆</a> &gt; <a href="${path}/frontVenue/venueDetail.do?venueId=${cmsVenue.venueId}" ><c:out escapeXml="true" value="${cmsVenue.venueName}"/></a> &gt; 视频点播</div>
            ...
            <div id="video-right">
	            <ul class="video-list">
	            	<c:forEach items="${cmsVideoList}" var="video">
			       	   <li activity-icon-url="${video.videoImgUrl}" onclick="showVideo('${video.videoId}','${cmsVenue.venueId}');">
		       	    ...
		       	    <script>
				       	 function showVideo(videoId,venueId){
			    	   			window.location.href="${path}/frontVenue/frontVenueVideo.do?videoId="+videoId+"&venueId="+venueId;
		    	   		 }
		       	    </script>
	            </ul>
	         </div>
