...
public class WechatAntiqueController {
    ...
    @RequestMapping(value = "/screenAppAntique")
    public String screenAppAntique(PaginationApp pageApp, HttpServletResponse response, String antiqueDynasty, String antiqueTypeName, String pageIndex, String pageNum, String venueId) throws Exception {
        String json = "";
        pageApp.setFirstResult(Integer.valueOf(pageIndex));
        pageApp.setRows(Integer.valueOf(pageNum));
        try {
            json = antiqueAppService.queryAppAntiqueList(antiqueTypeName, antiqueDynasty, pageApp, venueId);
        } catch (Exception e) {
            e.printStackTrace();
        }
        response.setContentType("text/html;charset=UTF-8");
