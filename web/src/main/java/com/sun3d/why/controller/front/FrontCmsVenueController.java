...
//            model.addObject("typeList", cmsTagService.getTagsByDictTagType("venue"));
    ...
    @ResponseBody
    public Map<String, Object> activityList(String venueId, Integer pageNum) {
        CmsActivity cmsActivity = new CmsActivity();
        cmsActivity.setVenueId(venueId);
        cmsActivity.setRows(4);
        //活动室显示数量每页为4个
        if (pageNum != null) {
        	cmsActivity.setPage(pageNum);
        }
        List<CmsActivity> activityList = cmsActivityService.queryRelatedActivity(cmsActivity, false);
        int activityCount = cmsActivityService.queryRelatedActivityCount(cmsActivity, false);
