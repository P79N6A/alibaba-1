...
public class BpInfoFrontController {
	...
	public ModelAndView bpInfoDetail(ModelAndView mv, String infoId,String module) {
		// 推荐位管理
	...
	public String chuanzhouList(String infoTagCode, String parentTagCode, HttpServletRequest request, Pagination page) {
		...
		request.setAttribute("parentTagCode", parentTagCode);
	 ...
	 * @return
	 */
	@RequestMapping("/czfrontindex")
	public ModelAndView czfrontindex(HttpServletRequest request,String module,String keyword) {
		...
		model.setViewName("index/chuanzhou/czfrontindex");
		return model;
	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/cztypelist")
	@ResponseBody
	public List<BpInfoTag> queryAreaList(String pid,String grade,String module) {
		List<BpInfoTag> list = new ArrayList<BpInfoTag>();
		//list = bpInfoTagServcie.queryParentTag(module);
		list = bpInfoTagServcie.queryChildTags(module);
		return list;
	}

	@RequestMapping("/ysjsList")
	public String chuanzhouListYsjs(BpInfo bpInfo, HttpServletRequest request, Pagination page) {
		// 设置每页12条数据
		page.setRows(12);
		List<BpInfo> infoList = bpInfoService.queryOtherInfoList(bpInfo, page);
		request.setAttribute("module", bpInfo.getModule());
		request.setAttribute("infoList", infoList);
		request.setAttribute("page", page);
		return "index/chuanzhou/czlistload";
	}
}
