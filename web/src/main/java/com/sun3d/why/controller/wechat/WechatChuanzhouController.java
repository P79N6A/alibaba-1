...
public class WechatChuanzhouController {
	...
	//h5端带上列表
	@RequestMapping("/chuanzhouList2")
	public ModelAndView chuanzhouList2(ModelAndView mv, String infoTagCode,HttpServletRequest request) {

		// 加载页面父标签
		List<BpInfoTag> parentTagList = bpInfoTagServcie.queryParentTag();
		mv.addObject("parentTagList", parentTagList);
		// 顶端子标签集合
		List<BpInfoTag> infoTagList = bpInfoTagServcie.queryChildTagByChildTag(infoTagCode);
		// 列子标签下资讯列表(首次进入列表页显示五条数据)
		// PaginationApp pageApp = new PaginationApp();
		...
		Map<String, String> sign = BindWS.sign(url, cacheService);
		request.setAttribute("sign", sign);
		mv.setViewName("wechat/bpChuanzhou/chuanzhouList2");
		return mv;
	}

	// 列表页下拉加载数据，传模块id
	@RequestMapping("loadChuanzhouList2")
	@ResponseBody
	public List<BpInfo> loadChuanzhouList2(String parentTagId, String startIndex, String pageSize) {
		PaginationApp pageApp = new PaginationApp();
		pageApp.setFirstResult(Integer.parseInt(startIndex));
		pageApp.setRows(Integer.parseInt(pageSize));
		List<BpInfo> infoList = bpInfoService.wechatQueryInfoList2(parentTagId, pageApp);
		return infoList;
	}
	...
	@RequestMapping("/wxczList")
	public ModelAndView wxczList(ModelAndView mv,String beipiaoinfoTag,String module) {

		// 顶端子标签集合
		//List<BpInfoTag> infoTagList = bpInfoTagServcie.queryChildTagByChildTag(infoTagCode);
		List<BpInfoTag> infoTagList = new ArrayList<BpInfoTag>();
		infoTagList = bpInfoTagServcie.queryParentTag(module);
		mv.addObject("beipiaoinfoTag", beipiaoinfoTag);
		mv.addObject("infoTagList", infoTagList);
		mv.addObject("module", module);
		//mv.setViewName("wechat/bpChuanzhou/chuanzhouList");
		mv.setViewName("wechat/bpChuanzhou/wxczList");
		return mv;
	}
	// 人文川州列表页(h5)下拉加载数据
	@RequestMapping("/loadwxazList")
	@ResponseBody
	public List<BpInfo> loadwxazList(String beipiaoinfoTag,String module, String startIndex, String pageSize) {
		PaginationApp pageApp = new PaginationApp();
		pageApp.setFirstResult(Integer.parseInt(startIndex));
		pageApp.setRows(Integer.parseInt(pageSize));
		List<BpInfo> infoList = bpInfoService.loadwxazList(beipiaoinfoTag,module, pageApp);
		return infoList;
	}
