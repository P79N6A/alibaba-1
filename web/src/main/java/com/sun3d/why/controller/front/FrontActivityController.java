...
public class FrontActivityController {
    ...
    public String frontActivityDetail(HttpServletRequest request) {
        try {
            ...
            int wantdoCount = 0;
            if(user != null){
            	wantdoCount = cmsUserWantgoMapper.queryAppUserWantCountById(new CmsUserWantgo(user.getUserId(),activityId));
            }
            request.setAttribute("wantdoCount", wantdoCount);
     ...
     * @return
     */
    @SysBusinessLog(remark = "web进入活动预订页面")
    ...
    public ModelAndView frontActivityBook(String activityId) {
        ...
        try {
            ...
            if (cmsActivity != null && cmsActivity.getActivityIsDel() == 1 && cmsActivity.getActivityState() == 6) {
                ...
                if(StringUtils.isNotBlank(cmsActivity.getActivityCustomInfo())){
                	JSONArray customInfojson = JSONArray.fromObject(cmsActivity.getActivityCustomInfo());
                	model.addObject("customInfojson", customInfojson);
                }
                return model;
            }
        } catch (Exception ex) {
    ...
    public Map showActivitySeatInfo(String activityId, String eventDateTime) throws Exception {
        ...
        if (org.apache.commons.collections.CollectionUtils.isEmpty(list) && cmsActivity.getActivitySalesOnline() == "Y") {
...
/*                if(Integer.valueOf(activitySeat.getSeatStatus()) == Constant.VENUE_SEAT_STATUS_OCCUPY){
    ...
    public ModelAndView updateActivityBookInfo(String activityId, String selectSeatInfo, Integer bookCount, String eventDateTime, String orderPhoneNo,String orderCustomInfo) {
        ModelAndView model = new ModelAndView();
        try {
            CmsActivity cmsActivity = activityService.queryFrontActivityByActivityId(activityId);
            if (cmsActivity != null) {
                ...
                if(StringUtils.isNotBlank(orderCustomInfo)){
                	JSONArray customInfojson = JSONArray.fromObject(orderCustomInfo);
                	model.addObject("customInfojson", customInfojson);
                }
                model.setViewName("index/activity/frontActivityBook");
                return model;
     ...
     * @return
     */
    @SysBusinessLog(remark = "web验证活动预定")
...
//            String checkRs = cacheService.checkActivitySeatStatus(activitySeatInfo, activitySeatInfo.getSeatIds(), bookCount, terminalUser.getUserId());
     ...
     * @return
     */
    @SysBusinessLog(remark = "web确定订单页面")
    ...
    public String preSaveActivityOder(HttpServletRequest request, CmsActivityOrder cmsActivityOrder, String[] seatId, Integer bookCount, String seatValues) {
        try {
            ...
            if(StringUtils.isNotBlank(saveSeatValues)){
            ...
            if(StringUtils.isNotBlank(cmsActivityOrder.getOrderCustomInfo())){
            	JSONArray customInfojson = JSONArray.fromObject(cmsActivityOrder.getOrderCustomInfo());
            	request.setAttribute("customInfojson", customInfojson);
            }
    ...
    public String saveActivityOderOver(HttpServletRequest request, String activityId,String orderValidateCode, String activityOrderId, String seatValues, String eventDateTime) {
        try {
    ...
    public Map<String, String> cancelUserOrder(HttpServletRequest request, String activityOrderId, String orderSeat, Integer cancelCount) {
        ...
        try {
            CmsTerminalUser terminalUser = (CmsTerminalUser) session.getAttribute("terminalUser");
            map = cmsActivityOrderService.cancelUserOrder(activityOrderId, terminalUser, orderSeat, cancelCount,null);
            return map;
        } catch (Exception ex) {
            map.put("success", "N");
            map.put("msg", ex.toString());
