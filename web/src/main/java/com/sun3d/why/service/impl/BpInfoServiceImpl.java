...
public class BpInfoServiceImpl implements BpInfoService {
	...
	@Override
	public List<BpInfo> queryOtherInfoList(BpInfo bpInfo, Pagination page) {
		// TODO Auto-generated method stub
		Map<String, Object> queryMap = new HashMap<>();

		if (StringUtils.isNotBlank(bpInfo.getBeipiaoinfoTag())) {
			queryMap.put("beipiaoinfoTag", bpInfo.getBeipiaoinfoTag());
		}
		if (StringUtils.isNotBlank(bpInfo.getYsjType())) {
			queryMap.put("ysjType", bpInfo.getYsjType());
		}
		if (StringUtils.isNotBlank(bpInfo.getModule())) {
			queryMap.put("module", bpInfo.getModule());
		}
		if (StringUtils.isNotBlank(bpInfo.getKeyword())) {
			queryMap.put("keyword", bpInfo.getKeyword());
		}
		queryMap.put("beipiaoinfoStatus", "1");
		if (page != null && page.getFirstResult() != null && page.getRows() != null) {
			// 查询出总数
			queryMap.put("firstResult", page.getFirstResult());
			queryMap.put("rows", page.getRows());
			page.setTotal(bpInfoMapper.queryInfoCountByMap(queryMap));
		}
		return bpInfoMapper.queryInfoListByMap(queryMap);
	}
	 ...
	 */
	@Override
	public List<BpInfo> wechatQueryInfoList2(String parentTagId, PaginationApp page) {
		Map<String, Object> queryMap = new HashMap<>();
		if (parentTagId!=null&&StringUtils.isNotBlank(parentTagId)) {
			queryMap.put("parentTagId", parentTagId);
		}
		if (page != null && page.getFirstResult() != null && page.getRows() != null) {
			// 查询出总数
			queryMap.put("firstResult", page.getFirstResult());
		...
		List<BpInfo> list = bpInfoMapper.queryBpInfoListByParentId(queryMap);
		return list;
	}
	...
	@Override
	public List<BpInfo> loadwxazList(String beipiaoinfoTag,String module, PaginationApp page) {
		// TODO Auto-generated method stub
		Map<String, Object> queryMap = new HashMap<>();
		if (StringUtils.isNotBlank(beipiaoinfoTag)) {
			queryMap.put("beipiaoinfoTag", beipiaoinfoTag);
		}
		...
		List<BpInfo> list = bpInfoMapper.querywxBpInfoListByCode(queryMap);
		return list;
	}
}
