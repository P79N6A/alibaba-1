...
public class CmsActivityServiceImpl implements CmsActivityService {
	...
	@Autowired
	private TicketMachineVenueMapper ticketMachineVenueMapper;
	...
	public String addActivity(final CmsActivity activity, final SysUser sysUser) {
		try {
			...
			if (activity != null) {
				...
				Integer activityIsReservation = activity.getActivityIsReservation() != null
						? activity.getActivityIsReservation() : 1;
	...
	public String editAppNavigationRecommendActivity(CmsActivity activity, Pagination page, SysUser user) {
		...
		// related.setRecommendColumnType(map.get("recommendColumnType") == null
		// ? 1 : Integer.parseInt(map.get("recommendColumnType").toString()));
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					cmsActivities2 = queryFrontActivityList(new CmsActivity(), "", null, null, null, null, page2, null,
							null);
				} catch (Exception e) {
					e.printStackTrace();
	 ...
	 */
	@Override
	public String editActivity(final CmsActivity cur, final SysUser sysUser, final String seatIds,
			final boolean hadOrder) {
		try {
			if (cur != null) {
				...
				Integer activityIsReservation = cur.getActivityIsReservation() != null ? cur.getActivityIsReservation()
						: 1;
				if (activityIsReservation == 1) {
				...
				if (activity != null && StringUtils.isNotBlank(activity.getActivityName())
						&& StringUtils.isNotBlank(cur.getActivityName())
						&& !activity.getActivityName().trim().equals(cur.getActivityName().trim()))
					// 验证活动名称
				...
				if (CollectionUtils.isNotEmpty(activityList)) {
					...
					for (CmsActivity cmsActivity : activityList) {
						...
						if (CollectionUtils.isNotEmpty(cmsActivityList)) {
							...
							// 设置过期时间为当前时间之后的24小时
							calendar.add(Calendar.HOUR_OF_DAY, 24);
							cacheService.setLikeActivityList(CacheConstant.APP_TOP_ACTIVITY + cmsActivity.getTagId(),
									cmsActivityList, calendar.getTime());
						}
					}
				}
			}
		...
		Runnable runner = new Runnable() {
			...
			public void run() {
				...
				if (CollectionUtils.isNotEmpty(list)) {
					...
					// 设置过期时间为当前时间之后的24小时
					calendar.add(Calendar.HOUR_OF_DAY, 24);
					cacheService.setLikeActivityList(CacheConstant.APP_RECOMMEND_CMS_ACTIVITY, list,
							calendar.getTime());
				}
				...
				if (CollectionUtils.isNotEmpty(activityList)) {
					...
					for (CmsActivity cmsActivity : activityList) {
						...
						if (CollectionUtils.isNotEmpty(cmsActivityList)) {
							...
							// 设置过期时间为当前时间之后的24小时
							calendar.add(Calendar.HOUR_OF_DAY, 24);
							cacheService.setLikeActivityList(
									CacheConstant.APP_TOP_CMS_ACTIVITY + cmsActivity.getTagId(), cmsActivityList,
									calendar.getTime());
						}
					}
				}
	...
	public List<CmsActivity> queryCmsActivityByAdminCondition(CmsActivity activity, Pagination page, SysUser sysUser) {
		...
		// 是否推荐
		if (activity != null && StringUtils.isNotBlank(activity.getActivityRecommend())
				&& "Y".equals(activity.getActivityRecommend())) {
			map.put("activityRecommend", "Y");
	...
	public List<CmsActivity> queryCmsActivityByCondition(CmsActivity activity, Pagination page, SysUser sysUser) {
		...
		} else if (activity != null && activity.getActivityIsDel() != null
				&& activity.getActivityIsDel() == Constant.NORMAL) {
	...
	public List<CmsActivity> queryPersonalActivityByCondition(CmsActivity activity, Pagination page, SysUser sysUser) {
		 ...
		 * StringUtils.isNotBlank(sysUser.getUserDeptPath())) {
	 ...
	 */
	@Override
	public List<CmsActivity> queryCollectActivity(CmsTerminalUser user, Pagination page, String activityName,
			PaginationApp pageApp) {
		Map<String, Object> map = new HashMap<String, Object>();
	 ...
	 */
	@Override
	public List<CmsActivity> queryActivityByCondition(CmsActivity activity, Pagination page, Date startDate,
	...
	@Override
	public List<CmsActivity> queryFrontActivityList(CmsActivity activity, String areaCode, String type, String theme,
			String activityStartTime, String activityEndTime, Pagination page, PaginationApp pageApp, String timeType)
			throws Exception {
		...
		if (page != null && page.getFirstResult() != null && page.getRows() != null) {
			map.put("firstResult", page.getFirstResult());
			map.put("rows", page.getRows());
			int total = queryFrontActivityListCount(activity, areaCode, type, theme, activityStartTime, activityEndTime,
					timeType);
			page.setTotal(total);
		}
	...
	// @Override
	// public List<Map> queryIndexListCollectViewNum(CmsActivity activity,
	// String areaCode,
	// String type, String theme,
	// String activityStartTime, String activityEndTime, Pagination page,
	// PaginationApp pageApp, String timeType) throws Exception {
	// SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
	// String nowDate = null;
	// Map map = new HashMap();
	// //1 代表今天 2代表明天 3代表本周末
	...
	// //位置
	// if (activity != null &&
	// StringUtils.isNotBlank(activity.getActivityLocation())) {
	// map.put("activityLocation", activity.getActivityLocation());
	// }
	// //区县代码
	// if (areaCode != null && !"".equals(areaCode)) {
	...
	//
	// //心情
	// if (activity.getActivityMood() != null &&
	// StringUtils.isNotBlank(activity.getActivityMood())) {
	// map.put("activityMood", "%" + activity.getActivityMood() + ",%");
	// }
	// //活动名称
	...
	// StringUtils.isNotBlank(activity.getActivityPrice())) {
	// // 1代表 0 2代表0到100 3代表100到200 4 代表200以上
	// if ("1".equals(activity.getActivityPrice())) {
	// // map.put("activityPrice", 0);
	// map.put("activityPrice", 0);
	...
	// //人群
	// if (activity != null &&
	// StringUtils.isNotBlank(activity.getActivityCrowd())) {
	// map.put("activityCrowd", "%" + activity.getActivityCrowd() + ",%");
	// }
	// //app活动名称模糊查询
	// if (activity != null &&
	...
	//
	// //网页分页
	// if (page != null && page.getFirstResult() != null && page.getRows() !=
	// null) {
	// map.put("firstResult", page.getFirstResult());
	// map.put("rows", page.getRows());
	// }
	...
	// activityMapper.queryIndexListCollectViewNum(map);
	//
	// return activityList;
	// }
	...
	@Override
	public List queryActivityListLoad(CmsActivity activity, String areaCode, String type, String theme,
			String orderType, String activityStartTime, String activityEndTime, Pagination page) throws Exception {
		Map map = new HashMap();
		// 位置
		if (activity != null && StringUtils.isNotBlank(activity.getActivityLocation())) {
			map.put("activityLocation", activity.getActivityLocation());
	...
	@Override
	public Integer queryFrontActivityListCount(CmsActivity activity, String areaCode, String type, String theme,
	...
	@Override
	public List<CmsActivity> queryActivityListByCity(String areaCode, String[] activityIds, String activityName,
			CmsActivity cmsActivity, Pagination page) {
		Map map = new HashMap();
	...
	/**
	 * app查询标签下当天是否有活动 app查询标签下附近是否有活动
	 *
	 * @param
	 * @param tagId
	 * @param dictCode
	 ...
	 */
	@Override
	public List<CmsActivity> queryAppCountByActivity(String tagId, String dictCode, String date,
			PaginationApp pageApp) {
		Map<String, Object> map = new HashMap<String, Object>();
	...
	/*
	 * @Override public List<CmsActivity> queryAppHotByActivity(CmsActivity
	 * activity, String areaCode, String activityStartTime, String
	 * activityEndTime, PaginationApp pageApp, String timeType) throws
	 * ParseException { SimpleDateFormat sf = new SimpleDateFormat(
	 * "yyyy-MM-dd HH:mm"); String nowDate = null; Map map = new HashMap(); //1
	 * 代表今天 2代表明天 3代表本周末 if (StringUtils.isNotBlank(timeType) &&
	 * !timeType.equals("")) { Date date = new Date(); nowDate =
	 * sf.format(date); if (timeType.equals("1")) { map.put("activityTime",
	 * nowDate); } else if (timeType.equals("2")) { Calendar cal =
	 * Calendar.getInstance(); cal.setTime(sf.parse(nowDate));
	 * cal.add(Calendar.DAY_OF_YEAR, +1); String nextDate =
	 * sf.format(cal.getTime()); map.put("activityTime", nextDate); } else if
	 * (timeType.equals("3")) { Calendar currentDate = Calendar.getInstance();
	 * currentDate.setFirstDayOfWeek(Calendar.MONDAY);
	 * currentDate.set(Calendar.HOUR_OF_DAY, 23);
	 * currentDate.set(Calendar.MINUTE, 59); currentDate.set(Calendar.SECOND,
	 * 59); currentDate.set(Calendar.DAY_OF_WEEK, Calendar.SUNDAY);
	 * map.put("activityTime", sf.format((Date) currentDate.getTime())); } }
	 * 
	 * //位置 if (activity != null &&
	 * StringUtils.isNotBlank(activity.getActivityLocation())) {
	 * map.put("activityLocation", activity.getActivityLocation()); } //区县代码 if
	 * (areaCode != null && !"".equals(areaCode) && !areaCode.equals("null")) {
	 * map.put("activityArea", areaCode + ",%"); } //主题 if (activity != null &&
	 * StringUtils.isNotBlank(activity.getActivityType())) {
	 * map.put("activityType", "%" + activity.getActivityType() + ",%"); } //心情
	 * if (activity.getActivityMood() != null &&
	 * StringUtils.isNotBlank(activity.getActivityMood())) {
	 * map.put("activityMood", "%" + activity.getActivityMood() + ",%"); }
	 * //活动名称 if (activity.getActivityName() != null &&
	 * StringUtils.isNotBlank(activity.getActivityName())) {
	 * map.put("activityName", "%" + activity.getActivityName() + "%"); } //活动时间
	 * if (activityStartTime != null && !StringUtils.isBlank(activityStartTime))
	 * { Calendar calendar = Calendar.getInstance(); calendar.setTime(new
	 * Date()); if ("1".equals(activityStartTime)) { calendar.set(Calendar.DATE,
	 * calendar.get(Calendar.DATE) - 7); } else if
	 * ("2".equals(activityStartTime)) { calendar.add(Calendar.MONTH, -1); }
	 * else if ("3".equals(activityStartTime)) { calendar.add(Calendar.MONTH,
	 * -3); } else if ("4".equals(activityStartTime)) {
	 * calendar.add(Calendar.YEAR, -1); } else {
	 * calendar.setTime(DateUtils.string2Date2(activityStartTime)); }
	 * activity.setStartTime(calendar.getTime()); String date =
	 * DateUtils.formatDate(calendar.getTime()); map.put("activityStartTime",
	 * date); } if (activityEndTime != null &&
	 * StringUtils.isNotBlank(activityEndTime)) { Calendar calendar =
	 * Calendar.getInstance();
	 * calendar.setTime(DateUtils.string2Date2(activityEndTime)); String date =
	 * DateUtils.formatDate(calendar.getTime()); map.put("activityEndTime",
	 * date); }
	 * 
	 * //人群 if (activity != null &&
	 * StringUtils.isNotBlank(activity.getActivityCrowd())) {
	 * map.put("activityCrowd", "%" + activity.getActivityCrowd() + ",%"); }
	 * //app活动名称模糊查询 if (activity != null &&
	 * StringUtils.isNotBlank(activity.getActivityName())) {
	 * map.put("activityName", "%" + activity.getActivityName() + "%"); }
	 * map.put("activityIsDel", 1); map.put("activityState", 6); //app分页 if
	 * (pageApp != null && pageApp.getFirstResult() != null && pageApp.getRows()
	 * != null) { map.put("firstResult", pageApp.getFirstResult());
	 * map.put("rows", pageApp.getRows()); int total =
	 * activityMapper.queryAppHotByActivityListCount(map);
	 * pageApp.setTotal(total); } List<CmsActivity> activityList =
	 * activityMapper.queryAppHotByActivity(map); return activityList; }
	 */

	/**
	 * @Par type 1周 2月 3季 4年 查询需要统计的活动数据
	 */
	 ...
	 */
	@Override
	public String refuseTuserActivityByActivityId(String activityId, Integer activityState, SysUser sysUser,
			String[] reason) {
		...
		if (StringUtils.isNoneBlank(activityId)) {
			...
			userMessageService.sendSystemMessage(Constant.SYSTEM_NOTICE_REFUSEUSER_ACTIVITY, params,
					cmsTerminalUser.getUserId());
			return Constant.RESULT_STR_SUCCESS;
		}
		return Constant.RESULT_STR_FAILURE;
	}
			...
			for (CmsActivity activity : cmsActivityList) {
				...
				if (StringUtils.isNotBlank(activity.getActivityStartTime())) {
					...
					if (startTimeInfo.length == 2) {
						...
						startEventTime = startTimeInfo[1].length() > 5 ? startTimeInfo[1].substring(0, 5)
								: startTimeInfo[1];
				...
				// 订单表中添加场次管理
				cmsActivityOrderMapper.updateOrderEventByActivityId(cmsActivityEvent.getEventId(),
	...
	public String addAPIActivity(final CmsActivity activity, final SysUser sysUser, final String seatIds) {
		try {
			if (activity != null) {
				...
				// Integer activityIsReservation =
				// activity.getActivityIsReservation() != null ?
				// activity.getActivityIsReservation() : 1;
				// if (activityIsReservation == 1) {
				// activity.setActivitySalesOnline("N");
				// activity.setActivityIsReservation(1);
				...
				// }
				// 子系统过来的活动都不能预定
				// activity.setActivityIsReservation(activity.getActivityIsReservation()
				// == null ? 1 : activity.getActivityIsReservation());
				// activity.setActivitySalesOnline(StringUtils.isBlank(activity.getActivitySalesOnline())
				// ? "N" : activity.getActivitySalesOnline());
				activity.setEventCount(activity.getActivityReservationCount());
				activityMapper.addCmsActivity(activity);
				...
				if (StringUtils.isNotBlank(activity.getEventStartTimes())
						&& StringUtils.isNotBlank(activity.getEventEndTimes())) {
				...
				if ((seatIds != null && StringUtils.isNotBlank(seatIds))
						|| (activity.getActivityReservationCount() != null
								&& activity.getActivityReservationCount() != 0)) {
					try {
						Runnable runnable = new Runnable() {
							@Override
							public void run() {
									...
									if (StringUtils.isNotBlank(activity.getActivityEndTime())) {
										cmsActivitySeatService.addActivitySeatInfo(activity.getActivityId(), seatIds,
												...
												new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
														.parse(activity.getActivityStartTime() + " 23:59:59"),
												activity, eventTimeInfo);
									} else {
										cmsActivitySeatService
												.addActivitySeatInfo(activity.getActivityId(), seatIds,
														activity.getActivityReservationCount(), sysUser,
														new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
																.parse(activityEndTimeStr + " 23:59:59"),
														activity, eventTimeInfo);
									}
	...
	@Override
	public String editActivityAPI(final CmsActivity cur, final SysUser sysUser, final String seatIds,
			final boolean hadOrder) {
		...
		try {
			if (cur != null) {
				...
				if (!hadOrder) {
					// Integer activityIsReservation =
					// cur.getActivityIsReservation() != null ?
					// cur.getActivityIsReservation() : 1;
					// if (activityIsReservation == 1) {
					// cur.setActivitySalesOnline("N");
					// cur.setActivityIsReservation(1);
					// } else if (activityIsReservation == 2) {
			...
			if (!"".equals(activityEndTimesStr)) {
				...
				cmsActivityEvent.setEventTime(dateStart[1].toString() + "-23:59");// +
																					// "-23:00");
			...
			if (((seatIds != null && StringUtils.isNotBlank(seatIds)) || cur.getActivityReservationCount() != null)
					&& !hadOrder) {
					...
					public void run() {
						try {
							if (StringUtils.isNotBlank(cur.getActivityEndTime())) {
								cmsActivitySeatService.addActivitySeatInfo(cur.getActivityId(), seatIds,
										cur.getActivityReservationCount(), sysUser,
										new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
												.parse(cur.getActivityEndTime() + " 23:59:59"),
										cur, eventTimeInfos);
							} else {
								cmsActivitySeatService.addActivitySeatInfo(cur.getActivityId(), seatIds,
										cur.getActivityReservationCount(), sysUser,
										new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
												.parse(cur.getActivityStartTime() + " 23:59:59"),
										cur, eventTimeInfos);
							}
						} catch (Exception e) {
							e.printStackTrace();
				...
				// cmsActivitySeatService.addActivitySeatInfo(cur.getActivityId(),
				// seatIds, cur.getActivityReservationCount(), sysUser, new
				// SimpleDateFormat("yyyy-MM-dd
				// HH:mm:ss").parse(cur.getActivityEndTime() +
				// ":00"),cur,eventTimeInfo);
			}
		} catch (Exception e) {
			logger.error("editActivity error {}", e);
			throw new RuntimeException(e);
	...
	public List<CmsActivity> queryAppRecommendCmsActivityByCondition(CmsActivity activity, Pagination page) {
		...
		if (activity != null) {
			...
			if (StringUtils.isNotBlank(activity.getActivityTheme())) {
				...
				// String resultStr =
				// sysDictMapper.queryDictNameByActivityTheme(
	...
	public List<CmsActivity> queryWillStartActivity(CmsActivity activity, Pagination page) {
		...
		/*
		 * Collections.sort(activityList, new Comparator<CmsActivity>(){ public
		 * int compare(CmsActivity arg0, CmsActivity arg1) { return
		 * arg0.getActivityStartTime().compareTo(arg1.getActivityStartTime()); }
		 * });
		 */
		return activityList;
	}
	 ...
	 *            活动id
	 * @param userId
	 *            用户id return 是否报名成功 (成功：success；失败：false)
	 */
	@Override
	 ...
	 * @return
	 */
	public List<CmsActivity> queryUserPublicActivityList(CmsTerminalUser cmsTerminalUser, Integer[] states,
			Pagination page) {
		Map map = new HashMap();
	...
	public String addActivity(final CmsActivity cmsActivity, CmsTerminalUser cmsTerminalUser, Integer count) {
		if (cmsActivity.getActivityState() != null && cmsActivity.getActivityState() != 3
				&& cmsActivity.getActivityState() != 8) {
			return "error info";
		}
		// 表示可以预定
		...
		String eventEndTimes[] = null;
		List<String> eventTimeList = new ArrayList<String>();
		if (StringUtils.isNotBlank(cmsActivity.getEventStartTimes())
				&& StringUtils.isNotBlank(cmsActivity.getEventEndTimes())) {
		...
		if (cmsActivity.getActivityIsReservation() == 2) {
			Runnable runnable = new Runnable() {
				...
				public void run() {
					try {
						cmsActivitySeatService
								.addActivitySeatInfoByFrontUser(cmsActivity.getActivityId(),
										cmsActivity.getEventCount(),
										new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
												.parse(cmsActivity.getActivityEndTime() + " 23:59:59"),
										cmsActivity, eventTimeInfo);
					} catch (Exception e) {
						e.printStackTrace();
						throw new RuntimeException(e);
					}
		...
		return activityMapper.addCmsActivity(cmsActivity) > 0 ? Constant.RESULT_STR_SUCCESS
				: Constant.RESULT_STR_FAILURE;
	}
	...
	public String editPublicActivity(final CmsActivity cmsActivity, final CmsTerminalUser cmsTerminalUser) {
		...
		if (preActivity != null && StringUtils.isNotBlank(preActivity.getActivityName())
				&& StringUtils.isNotBlank(cmsActivity.getActivityName())
				&& !preActivity.getActivityName().trim().equals(cmsActivity.getActivityName().trim()))
			// 验证活动名称
		...
		return activityMapper.editCmsActivity(cmsActivity) > 0 ? Constant.RESULT_STR_SUCCESS
				: Constant.RESULT_STR_FAILURE;
	}
	...
	public String recommendActivity(String id, SysUser user, String platform) {
		...
		if ("Y".equals(activity.getActivityRecommend())) {
			activity.setActivityRecommend("N");
			activity.setActivityRecommendTime(null);
	...
	public String setRightToRedisAndDataBase(String activityId, String eventId) throws Exception {
		...
		//
		// //计算出可以被预定的剩余票数
		// int effectiveTicketCount =
		// cmsActivityOrderMapper.queryEffectiveTicketCountByActivityId(activityId,
		// eventId);
		// CmsActivityEvent cmsActivityEvent =
		// cmsActivityEventService.queryByEventId(eventId);
		// //自由入座的情况
		// if ("N".equals(cmsActivity.getActivitySalesOnline()) &&
		// cmsActivity.getActivityIsReservation() == 2) {
		...
		// cacheService.updateSeatCountByIdAndTime(activityId,
		// df.parse(cmsActivityEvent.getEventDate() + " 23:59"),
		// cmsActivityEvent.getEventDateTime(),
		// String.valueOf(effectiveTicketCount));
		/// * }*/
		...
		// map.put("activityId", activityId);
		// //查询活动座位模板
		// List<CmsActivitySeat> seatList =
		// cmsActivitySeatService.queryCmsActivitySeatCondition(map);
		// //
		// List<CmsActivitySeat> newSeatList = new ArrayList<CmsActivitySeat>();
		// //查询已经产生的有效订单
		// map.put("eventId", eventId);
		// List<CmsActivityOrderDetail> orderDetailList =
		// cmsActivityOrderDetailMapper.queryCmsActivityOrderDetailByActivityIdAndEventId(map);
		...
		// seatStatuMap.put(orderDetail.getSeatCode(),
		// orderDetail.getSeatStatus());
		// }
		// /* if (orderDetailList != null && orderDetailList.size() > 0) {*/
		...
		// //重新将座位信息放至内存中
		// DateFormat df = new SimpleDateFormat("yyyy-MM-dd 23:59");
		// cacheService.setValueToRedis(activityId.getBytes(),
		// ListTranscoder.serialize(eventMap),
		// df.parse(cmsActivityEvent.getEventDate() + " 23:59"));
		// //设置nei座位的票数]
		// /*
		// cacheService.setSeatCount(activityId,String.valueOf(effectiveTicketCount),df.parse(cmsActivityEvent.getEventDate()
		// + " 23:59"))*/
		// String key = (CacheConstant.ACTIVITY_TICKET_COUNT + activityId + "_"
		// + cmsActivityEvent.getEventDateTime());
		// cacheService.setValueToRedis(key,
		// String.valueOf(effectiveTicketCount),
		// df.parse(cmsActivityEvent.getEventDate() + " 23:59"));
		// cmsActivityEvent.setAvailableCount(effectiveTicketCount);
		// //修改数据库数量
		...
		// cacheService.setActivitySeat(activityId, seatList
		// ,df.parse(cmsActivityEvent.getEventDate()),cmsActivity,eventDateTimeList);
		// }*/
		// }
		return Constant.RESULT_STR_SUCCESS;
	}
	...
	public List<ActivityForCompare> queryOngoingActivityCanBook() {
		...
		if (cmsActivities != null && cmsActivities.size() > 0) {
			for (CmsActivity cmsActivity : cmsActivities) {
				...
				// 应该剩余的余票数
				Integer effectiveTicketCount = cmsActivityOrderMapper.queryEffectiveTicketCountByActivityId(activityId,
						cmsActivity.getEventIds());
				...
				if (redisCount != dataBaseCount || redisCount != effectiveTicketCount) {
					...
					// 活动预定的总票数 有效的票数
					Integer bookCount = cmsActivityOrderMapper
							.queryBookTicketCountByActivityId(cmsActivity.getActivityId(), cmsActivity.getEventIds());
	...
	public Map<String, Object> getSeatInfoByList(List<CmsActivitySeat> activitySeatList) {
		...
		for (int i = 0; i < activitySeatList.size(); i++) {
			...
			/*
			 * //判断是否在放票时间断 if ((activitySeat.getSeatOpenTime() != null &&
			 * activitySeat.getSeatOpenTime().after(new Date()))){
			 * seatInfoBuilder.append(normalSeat + "[" + row + "_" + column +"-"
			 * +activitySeat.getSeatVal() + "]"); vipInfoBuilder.append( row +
			 * "_" + column + ","); } else {
			 */
			if (Integer.valueOf(activitySeat.getSeatStatus()) == Constant.VENUE_SEAT_STATUS_NORMAL) {
				seatInfoBuilder.append(
						normalSeat + "[" + row + "_" + column + "-" + activitySeat.getSeatVal().split("_")[1] + "]");
				canBookCount++;
			}
			...
			/*
			 * if(Integer.valueOf(activitySeat.getSeatStatus()) ==
			 * Constant.VENUE_SEAT_STATUS_OCCUPY){
			 * seatInfoBuilder.append(normalSeat + "[" + row + "_" + column +"-"
			 * +activitySeat.getSeatVal() + "]"); vipInfoBuilder.append( row +
			 * "_" + column + ","); }
			 */
				...
				if (httpResponseText.getHttpCode() == 200) {
					...
					if (Boolean.parseBoolean(data.get("status") == null ? "false" : data.get("status").toString())) {
						...
						Integer ticketCount = Integer
								.valueOf(data.get("ticketCount") == null ? "0" : data.get("ticketCount").toString());
						map.put("seatInfo", seatInfo);
						...
						// 得到场次id
						CmsActivityEvent cmsActivityEvent = cmsActivityEventService
								.queryEventByActivityAndTime(cmsActivity.getActivityId(), eventDateTime);
	...
	public Map queryActivitySeatInfo(String activityId, String eventDateTime) throws Exception {
		...
		if ("1".equals(cmsActivity.getSysNo())) {
			...
			// 得到场次id
			CmsActivityEvent cmsActivityEvent = cmsActivityEventService.queryEventByActivityAndTime(activityId,
					eventDateTime);
			if (cmsActivityEvent != null) {
			...
			if (list != null) {
				...
				String maintananceInfo = map.get("maintananceInfo") == null ? ""
						: map.get("maintananceInfo").toString();
	...
	public Map queryFrontDetailInfo(String activityId) throws Exception {
		...
		if ("1".equals(cmsActivity.getSysNo()) && StringUtils.isNotBlank(cmsActivity.getSysId())) {
			...
			if (map1 != null && "Y".equals(map1.get("success"))) {
				Integer ticketCount = Integer
						.valueOf(map1.get("ticketCount") == null ? "0" : map1.get("ticketCount").toString());
		...
		try {
			...
			/*if(cmsActivity.getCancelEndTime() != null){
				cmsActivity.setActivityEndTime(new SimpleDateFormat("yyyy-MM-dd HH:mm").format(cmsActivity.getCancelEndTime()));
			}*/
			
			if(!"".equals(cmsActivity.getCancelEndTime())&&cmsActivity.getCancelEndTime()!=null){
				if(cmsActivity.getCancelEndTime().before(new Date())){
					map.put("isOver", "Y");
				}else{
					map.put("isOver", "N");
				}
				...
				if (StringUtils.isNotBlank(cmsActivity.getActivityEndTime())) {
					if (new SimpleDateFormat("yyyy-MM-dd HH:mm")
							.parse(cmsActivity.getActivityEndTime() + " " + endDate.split("-")[0]).before(new Date())) {
						map.put("isOver", "Y");
					} else {
						map.put("isOver", "N");
					}
					...
					if (new SimpleDateFormat("yyyy-MM-dd HH:mm")
							.parse(cmsActivity.getActivityStartTime() + " " + endDate.split("-")[0]).before(new Date())) {
						map.put("isOver", "Y");
					} else {
						map.put("isOver", "N");
					}
	 ...
	 * @return
	 */
	public List<CmsActivity> queryIndexActivityByCondition(CmsActivity activity, Pagination page, String isWeekend,
			String chooseType, String sortType, String bookType, CmsTerminalUser terminalUser) {
		Map map = new HashMap();
		//
		...
		if (page != null && page.getFirstResult() != null && page.getRows() != null) {
			...
			page.setTotal(activityMapper.queryCountActivityByCondition(map));
		}
	...
	@Override
	public List queryActivityListTicketLoad(CmsActivity activity, String machineCode, Pagination page)
			throws Exception {
		...
		if (StringUtils.isNotBlank(machineCode)) {
			...
			List<TicketMachineVenue> venueList = ticketMachineVenueMapper.selectTicketMachineVenueByModel(machineCode,
					null);

			if (venueList.size() > 0) {

				String venueId=venueList.get(0).getVenueId();
		...
		if (page != null && page.getFirstResult() != null && page.getRows() != null) {
			map.put("firstResult", page.getFirstResult());
			map.put("rows", page.getRows());
			int total =activityMapper. queryActivityListTicketLoadCount(map);
			page.setTotal(total);
		}

		return activityMapper.queryActivityListTicketLoad(map);

	}

	@Override
	public List<CmsActivity> pcnewActivity(String activityType,int num) {
		...
		map.put("activityIsDel", 1);
		map.put("activityState", 6);
		map.put("orderBy", "a.PUBLIC_TIME DESC");
		
		map.put("firstResult", 0);
		map.put("rows", num);
		List<CmsActivity> list = activityMapper.queryPcnewActivity(map);
		return list;
	}

	@Override
	public List<CmsActivity> getPctuijianActivity(List<String> ids) {
		// TODO Auto-generated method stub
		
		return activityMapper.queryPctuijianActivity(ids);
	}
	public List<CmsActivity> queryCmsActivityUnion(String id,Integer count){
		return activityMapper.queryCmsActivityUnion(id,count);
	}


	public List<CmsActivity> queryCmsActivityUnionPage(String id,Integer pageIndex){
		return activityMapper.queryCmsActivityUnion(id,pageIndex);
	}

	@Override
    @Transactional
	public List<CmsActivity> queryCmsActivityByMember(CmsMemberBO bo) {
		List<CmsActivity> list =  activityMapper.queryCmsActivityByMember(bo);
		bo.setShowPage(false);
		int total =   activityMapper.queryCmsActivityByMember(bo).size();
        bo.setTotal(total);
		return list;
	}

	@Override
	public List<CmsActivity> queryRelatedActivity(CmsActivity cmsActivity, boolean excludeFlag) {
		Map<String,Object> map = new HashMap<String, Object>();
		try{
			map.put("activityIsDel", Constant.NORMAL);
			map.put("activityState", Constant.PUBLISH);
			if(StringUtils.isNotBlank(cmsActivity.getVenueId())){
                map.put("venueId", cmsActivity.getVenueId());
            }
			map.put("firstResult", cmsActivity.getFirstResult());
            map.put("rows", cmsActivity.getRows());
            List<CmsActivity> list = cmsActivityMapper.queryRelatedActivity(map);
            return list;
		} catch (Exception e){
            logger.error("查询推荐活动信息出错!",e);
        }
		return null;
	}

	@Override
	public int queryRelatedActivityCount(CmsActivity cmsActivity, boolean excludeFlag) {
		Map<String,Object> map = new HashMap<String, Object>();
		try{
			map.put("activityIsDel", Constant.NORMAL);
			map.put("activityState", Constant.PUBLISH);
			...
			int total = cmsActivityMapper.queryRelatedActivityCount(map);
			return total;
		} catch (Exception e){
            logger.error("查询推荐活动数量信息出错!",e);
        }
		return 0;
	}

	@Override
	public List<CmsActivity> queryCalendarAdvert(String Time) {
		// TODO Auto-generated method stub
		Map map = new HashMap();
	/*	if(StringUtils.isNotBlank(activityType)){
			map.put("activityType", activityType);
		...
		List<CmsActivity> list = activityMapper.queryCalendarAdvert(map);
		return list;
	}
}
