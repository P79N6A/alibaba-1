...
public class CmsRoomOrderServiceImpl implements CmsRoomOrderService {
    ...
    @Autowired
    private CmsActivityRoomMapper cmsRoomMapper;
    ...
    public boolean cancelRoomOrder(String roomOrderId,String userName){
        ...
        try {
            ...
            if(cmsRoomOrder.getBookStatus()<2){
                ...
                CmsActivityRoom cmsRoom= cmsRoomMapper.queryCmsActivityRoomById(cmsRoomOrder.getRoomId());
                
                CmsVenue cmsVenue=cmsVenueMapper.queryVenueById(cmsRoom.getRoomVenueId());
                		...
                		&&cmsRoomBook.getUserId().equals(cmsRoomOrder.getUserId())&&cmsRoomBook.getBookStatus()==2)
                {
                    ...
                    //发送短信--阿里大鱼
                    SmsUtil.cancelRoomOrder(cmsRoomOrder.getUserTel(), map);
                }
	...
	public int checkPass(String roomOrderId,SysUser sysUser) {
		...
		try {
			...
			CmsActivityRoom cmsRoom= cmsRoomMapper.queryCmsActivityRoomById(cmsRoomBook.getRoomId());
            ...
            String venue= cmsVenue.getVenueName();
            
            String activity=cmsRoom.getRoomName();
               
            String time=DateUtils.formatDate(cmsRoomBook.getCurDate())+","+cmsRoomBook.getOpenPeriod();
			...
			for (CmsRoomOrder cmsRoomOrder : allCmsRoomOrderList) {
				...
				if(cmsRoomOrder.getRoomOrderId().equals(roomOrderId))
				{
					...
					if(r>0){
    		        	...
    		        	map.put("time",time);
    		        	map.put("ticketCode",cmsRoomOrder.getValidCode());
    		        	
    		            SmsUtil.passRoomOrder(cmsRoomOrder.getUserTel(), map);
					}
				...
				else
				{
							...
							if(r>0)
							{
		    		            ...
		    		            SmsUtil.autoCancelRoomOrder(cmsRoomOrder.getUserTel(), map);
