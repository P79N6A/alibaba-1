...
public class CmsTerminalUserServiceImpl implements CmsTerminalUserService {
    ...
    public List<CmsTerminalUser> queryTerminalUserByCondition(CmsTerminalUser user, Pagination page){
        ...
        List<CmsTerminalUser>userList=userMapper.queryTerminalUserByCondition(map);
        ...
        for (CmsTerminalUser cmsTerminalUser : userList) {
        	...
        	if(userIntegral!=null)
        		...
        		cmsTerminalUser.setIntegralNow(0);
		}
...
/*                if(StringUtils.isNotBlank(user.getUserName())){
                ...
                /*// 验证密码相同
                if(!user.getUserPwd().equals(confirmPassword)){
                ...
                if(StringUtils.isNotBlank(user.getUserMobileNo())){
                    ...
                    if(terminalUserIsExists(terminalUser)){
                        if(user.getSourceCode()==1){
                            //代表图书馆用户注册 并且所填写手机号已存在 2018/7/12
                            Map<String, Object> map = new HashMap<String, Object>();
                            map.put("userMobileNo", user.getUserMobileNo());
                            //map.put("userPwd",MD5Util.toMd5(user.getUserPwd()));
                            terminalUser = userMapper.queryByMap(map);
                            if(terminalUser!=null){
                                //手机号已存在用户自动与读者证注册用户绑定 2018/7/12
                                CmsTerminalUser cardUser = new CmsTerminalUser();
                                cardUser.setReaderCard(user.getReaderCard());
                                cardUser.setSourceCode(user.getSourceCode());
                                cardUser.setUserId(terminalUser.getUserId());
                                cardUser.setUserPwd(MD5Util.toMd5(user.getUserPwd()));
                                cardUser.setUserName(user.getUserName());
                                userMapper.editTerminalUserById(cardUser);
                ...
                if(user.getSourceCode()==null||user.getSourceCode()!=1) {
                    // 验证身份证号
        ...
        try{
            if(user != null) {
                ...
                if(terminalUser!=null) {
                   ...
                   /* if (StringUtils.isNotBlank(user.getUserName())) {
                        if (!user.getUserName().equals(terminalUser.getUserName())) {
                ...
                if(!user.getUserPwd().equals(confirmPassword)){
                    ...
                    // 验证身份证号
                        /*if (StringUtils.isNotBlank(user.getUserCardNo())) {
                            if (!user.getUserCardNo().equals(terminalUser.getUserCardNo())) {
                                CmsTerminalUser terminal = new CmsTerminalUser();
                                terminal.setUserCardNo(user.getUserCardNo());
                                if (terminalUserIsExists(terminalUser)) {
                                    return Constant.RESULT_STR_CARD_NO_REPEAT;
                                }
                            }
     ...
     */
    @Override
    public List<CmsTerminalUser> queryTeamTerminalUserByTuserId(CmsApplyJoinTeam applyJoinTeam,String userName, Pagination page){
        Map<String, Object> map = new HashMap<String, Object>();
            ...
            try{
                ...
                if(CollectionUtils.isNotEmpty(userTagList)) {
                    ...
                    if (activityThemeTagId.length() > 0) {
                        terminalUser.setActivityThemeTagId(activityThemeTagId.substring(0, activityThemeTagId.length() - 1));
                    }
                }
            }catch (Exception e){
                e.printStackTrace();
     ...
     * @param type(回调地址，判断来源)
     * @return
     */
    @Override
    public Map<String,Object> sendSmsCode(final String userMobileNo,String userName,String userPwd,Integer userSex,HttpServletRequest request) throws ParseException {
    ...
    public String saveUser(CmsTerminalUser user, String code,HttpSession session) {
        ...
        //app修改用户信息
       /* else{
            CmsTerminalUser terminalUser = this.queryTerminalUserByCode(code,user.getUserMobileNo());
            user.setUserPwd(MD5Util.toMd5(user.getUserPwd()));
            user.setCreateTime(new Date());
            user.setUserIsDisable(Constant.USER_IS_ACTIVATE);
    ...
    public String saveRegUser(CmsTerminalUser user, String code,HttpSession session) {
        ...
        if(tuser!=null){
            //短信码验证失败
            if(!code.equals(tuser.getRegisterCode())&&!code.equals("010203")){
                return Constant.SmsCodeErr;
            }
            if(!user.getUserMobileNo().equals(tuser.getUserMobileNo())){
                return Constant.RESULT_STR_FAILURE;
...
//          code = send.sendSmsMessage(smsConfig.getSmsUrl(),smsConfig.getuId(),smsConfig.getPwd(),userMobileNo,smsContent);
    ...
    public void sendSmsMessage(String userMobileNo,String smsContent,SmsConfig config){
        ...
        try{
                //new SmsSend().sendSmsMessage(config.getSmsUrl(), config.getuId(), config.getPwd(), userMobileNo, smsContent);
            }catch (Exception e){
                logger.info("sendSms error", e);
            }
        }
...
/*        Map<String, Object> param = new HashMap<String, Object>();
        ...
        // 当天发送过
        if (extUser.getLastSendSmsTime().getTime() >= start.getTime() && extUser.getLastSendSmsTime().getTime() < end.getTime()) {
...
/*            if(!"100".equals(sendSmsMessage(userMobileNo,smsContent))){ // 发送未成功返回，否则更新
  ...
  /*  @Override
    public int editAppTerminalUserById(CmsTerminalUser terminalUser) {
     ...
     */
    @Override
    public Map<String, Object> sendForgetSmsCode(final String userMobileNo, String from,HttpSession session) throws ParseException {
        Map<String,Object> result = new HashMap<String, Object>();
        if(!CommonUtil.isMobile(userMobileNo)){
            result.put("result",Constant.RESULT_STR_FAILURE);
            return  result ;
    ...
    @Override
    public Integer editUserByReaderCard(Map<String, Object> params){
        return userMapper.editUserByReaderCard(params);
    }

    @Override
    public CmsTerminalUser queryUserByReaderCard(String readerCard) {
        return userMapper.queryUserByReaderCard(readerCard);
    }
}
