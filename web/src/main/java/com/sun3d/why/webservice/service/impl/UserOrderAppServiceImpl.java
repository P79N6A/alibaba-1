		...
		try {
			...
			if (CollectionUtils.isNotEmpty(userActivityList)) {
			    for (CmsActivityOrder activityOrder : userActivityList) {
			    	...
			    	mapUserActivity.put("orderPaymentStatus", activityOrder.getOrderPaymentStatus());
			...
			e.printStackTrace();
			return JSONResponse.toAppResultObject(1, e.getMessage(), MapRoomList);
		}
...
//        }
	...
	public String queryAppUserActivityOrderDetail(String userId,String activityOrderId) throws Exception {
		...
		try {
			...
			CmsActivity cmsActivity = activityMapper.queryCmsActivityByActivityId(activityOrder.getActivityId());
			...
			mapUserActivity.put("activityIsDel", cmsActivity.getActivityIsDel());
			mapUserActivity.put("cancelEndTime", cmsActivity.getCancelEndTime());
            ...
            mapUserActivity.put("orderPrice", activityOrder.getOrderPrice()!= null ?activityOrder.getOrderPrice():"");
        	mapUserActivity.put("activityPayPrice", cmsActivity.getActivityPayPrice());
        	mapUserActivity.put("orderPaymentStatus", activityOrder.getOrderPaymentStatus());
            ...
            mapUserActivity.put("orderPayType", activityOrder.getOrderPayType());
			//生成活动订单id 进行取消活动订单
			mapUserActivity.put("activityOrderId", activityOrder.getActivityOrderId() != null ? activityOrder.getActivityOrderId() : "");
			mapUserActivity.put("orderLine",sbLine.toString());
			...
			mapUserActivity.put("activitySalesOnline",activityOrder.getActivitySalesOnline()!=null?activityOrder.getActivitySalesOnline():"");
			mapUserActivity.put("activityIsReservation",activityOrder.getActivityIsReservation()!=null?activityOrder.getActivityIsReservation():"");
			mapUserActivity.put("now",new Date().getTime());
			
			String activityIsFree=activityOrder.getActivityIsFree();
			// 显示激活码
			boolean showCode=false;
			if("3".equals(activityIsFree)){
				//支付状态 0.无需支付 1.待支付 2.支付完成 3.申请退款 4.退款成功
				Short orderPaymentStatus=activityOrder.getOrderPaymentStatus();
			...
			if(showCode){
				...
				mapUserActivity.put("orderCustomInfo", activityOrder.getOrderCustomInfo() != null ? activityOrder.getOrderCustomInfo() : "");
			}	
            ...
            for (CmsActivityOrder activityOrder : userActivityList) {
                ...
                if (StringUtils.isNotBlank(activityOrder.getSeatStatus())) {
                    ...
                    for (int i = 0; i < activitySeatStatus.length; i++) {
                        //1.预定成功  3.已出票
                        if (Integer.valueOf(activitySeatStatus[i]) == 1 || Integer.valueOf(activitySeatStatus[i])==3
                                || Integer.valueOf(activitySeatStatus[i])==4) {
                            seat=activityOrder.getSeats() != null ? StringUtils.split(activityOrder.getSeats(),",")[i] : "";
                            sbSeat.append(seat+",");
                            //订单票数
                            j++;
...
//        }
	...
	@Override
	public String queryAppUserOrderByOrderPhoneNo(PaginationApp pageApp, String orderPhoneNo) throws Exception {
		  List<Map<String, Object>> MapActivityList = new ArrayList<Map<String, Object>>();
	        List<Map<String, Object>> MapRoomList = new ArrayList<Map<String, Object>>();
	        Map<String, Object> map = new HashMap<String, Object>();
...
//	        }
	...
	@Override
	public String queryAppUserPayOrderByUserId(PaginationApp pageApp, String userId) throws Exception {
		   List<Map<String, Object>> MapActivityList = new ArrayList<Map<String, Object>>();
	        List<Map<String, Object>> MapRoomList = new ArrayList<Map<String, Object>>();
	        Map<String, Object> map = new HashMap<String, Object>();
	        map.put("userId", userId);
	        ...
	        //获取我的活动订单信息
			try {
				List<CmsActivityOrder> userActivityList = cmsActivityOrderMapper.queryAppActivityPayOrderByUserId(map);
				if (CollectionUtils.isNotEmpty(userActivityList)) {
				    for (CmsActivityOrder activityOrder : userActivityList) {
				        Map<String, Object> mapUserActivity = new HashMap<String, Object>();
				        mapUserActivity.put("activityId", activityOrder.getActivityId() != null ? activityOrder.getActivityId() : "");
