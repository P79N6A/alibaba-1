...
public class ActivityAppServiceImpl implements ActivityAppService {
    ...
    public String queryAppActivityById(String activityId, String userId) {
       ...
       String date = sdf.format(new Date());
        ...
        if (cmsActivity != null) {
            ...
            String activityIconUrl = cmsActivity.getActivityIconUrl();
            if (StringUtils.isNotBlank(activityIconUrl)) {
    ...
    public String queryAppCmsActivityById(String activityId, String userId) {
        ...
        if (cmsActivity != null) {
           ...
           // } else {
            ...
            mapActivity.put("activityPayPrice", cmsActivity.getActivityPayPrice() != null ? cmsActivity.getActivityPayPrice() : "");
            ...
            if (cmsActivity.getActivityIsFree() == 1) {
                if (cmsActivity.getActivityIsReservation() == 1) {
                    mapActivity.put("activityTips", "<font color='#FA8808' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'> 温馨提示：</font><font color='#262626' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'>本活动无需在文化云平台预约，具体参与方式请以主办方公布为准，您可拨打电话" +
                            "(" + mapActivity.get("activityTel") + ")进行咨询</font>");
                } else if (cmsActivity.getActivityIsReservation() == 2) {
                ...
                if (cmsActivity.getActivityIsReservation() == 1) {
                    mapActivity.put("activityTips", "<font color='#FA8808' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'>温馨提示：</font><font color='#262626' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'>本活动无需在文化云平台预约，需要收费，具体参与方式请以主办方公布为准，您可拨打电话" +
                            "(" + mapActivity.get("activityTel") + ")进行咨询</font>");
                } else if (cmsActivity.getActivityIsReservation() == 2) {
            	  ...
            	  mapActivity.put("activityTips","<font color='#262626' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'>温馨提示：</font>"
         	 		+ "<font color='#262626' style='font-family:\"STYuanti-SC-Light\";font-size:\"26px\"'>1.您需要在下单成功后15分钟内付款，逾期订单即被取消。<br>2.活动名额有限，付款视为占用名额，"
            ...
            List<CmsTagSub> cmsTagSubList=cmsTagSubMapper.queryRelateTagSubList(cmsActivity.getActivityId());
			 
			List<CmsTagSub> subList= new ArrayList<CmsTagSub>();
			
			for (CmsTagSub cmsTagSub : cmsTagSubList) {
				
				subList.add(cmsTagSub);
			}
			
			mapMood.put("subList", subList);
            ...
            mapActivity.put("activitySupplementType", cmsActivity.getActivitySupplementType()!= null ? cmsActivity.getActivitySupplementType() : "");
            
            //积分判断
            if (StringUtils.isNotBlank(userId)) {
                SysUserIntegralReqVO vo = new SysUserIntegralReqVO();
            	vo.setUserId(userId);
                HttpResponseText res = CallUtils.callUrlHttpPost(staticServer.getPlatformDataUrl()+"integral/getUserIntegralByUserId",vo);
        		JSONObject jsonObject = JSON.parseObject(res.getData());
        		Integer integralNow = Integer.parseInt(jsonObject.get("data").toString());
    ...
    public String queryUserAppCollectAct(String userId, PaginationApp pageApp) {
        ...
        if (activityList.size() > 0) {
            for (CmsActivity activity : activityList) {
                ...
                //子站调用统一数据格式
                mapActivity.put("relateId", activity.getActivityId() != null ? activity.getActivityId() : "");
                mapActivity.put("name", activity.getActivityName() != null ? activity.getActivityName() : "");
                mapActivity.put("info", activity.getActivityStartTime()+(activity.getActivityEndTime()!=null?(" 至 "+activity.getActivityEndTime()):""));
                mapActivity.put("iconUrl", StringUtils.isNotBlank(activity.getActivityIconUrl()) ? staticServer.getStaticServerUrl() + activity.getActivityIconUrl() : "");
    ...
    public String queryAppActivitySeatsById(String activityId, String userId, String activityEventimes) {
        ...
        if (cmsActivity != null) {
            ...
            activityMap.put("activityName", cmsActivity.getActivityName());
            // 嘉定数据
    ...
    public String appActivityOrderByCondition(String activityId, String userId, String activityEventIds, String bookCount, String orderMobileNum,
                                              String orderPrice, String activityEventimes, String seatId, String seatValues, String orderName, String orderIdentityCard, String costTotalCredit) {
        ...
        cmsActivityOrder.setSeats(seatId);
...
//                    JmsResult jmsResult = activityBookClient.bookActivitySeat(activitySeatInfo, cacheService);
                        if (!Constant.RESULT_STR_FAILURE.equals(counts)) {
                            ...
                            if (StringUtils.isNotBlank(userId) && Integer.parseInt(costTotalCredit) > 0) {
                                ...
                                if (count > 0 && Integer.parseInt(costTotalCredit) > 0) {
                                    ...
                                    userIntegralDetail.setIntegralFrom("系统扣除活动预订所需积分，订单号：" + cmsActivityOrder.getOrderNumber());
        ...
        try {
            if (CollectionUtils.isNotEmpty(activityList)) {
                ...
                for (CmsActivity activity : activityList) {
                    ...
                    String activityIconUrl = activity.getActivityIconUrl();
                    if (StringUtils.isNotBlank(activityIconUrl)) {
    ...
    public String queryAppActivityTagList(String userId) {
        ...
        List<Map<String, Object>> mapTypelist = new ArrayList<Map<String, Object>>();
        List<CmsActivity> activityThemeList = queryActivityThemeByCode(Constant.ACTIVITY_TYPE);
        ...
        try {
            if (CollectionUtils.isNotEmpty(activityList)) {
                for (CmsActivity activity : activityList) {
                    ...
                    mapActivity.put("activityPayPrice", activity.getActivityPayPrice() != null ? activity.getActivityPayPrice() : "");
                    mapActivity.put("activityIsFree", activity.getActivityIsFree() != null ? activity.getActivityIsFree() : 1);
                    mapActivity.put("activityStartTime", activity.getActivityStartTime() != null ? activity.getActivityStartTime() : "");
                    mapActivity.put("activityEndTime", activity.getActivityEndTime() != null ? activity.getActivityEndTime() : "");
                    mapActivity.put("tagId", activity.getTagId() != null ? activity.getTagId() : "");
                    mapActivity.put("tagName", activity.getTagName() != null ? activity.getTagName() : "");
                    mapActivity.put("venueName", activity.getVenueName() != null ? activity.getVenueName() : "");
                    mapActivity.put("activityUpdateTime", activity.getActivityUpdateTime() != null ? DateUtils.convertToSS(activity.getActivityUpdateTime()) : "");
                    mapActivity.put("activityCreateTime", activity.getActivityCreateTime() != null ? DateUtils.convertToSS(activity.getActivityCreateTime()) : "");
                    mapActivity.put("cancelEndTime", activity.getCancelEndTime() != null ? DateUtils.convertToSS(activity.getCancelEndTime()) : "");

                    if(activity.getActivityIsReservation()!=null){
                    ...
                    //标签列表
                    QueryRelateTagSubListVO vo = new QueryRelateTagSubListVO();
                    vo.setRelateId(activity.getActivityId());
                    HttpResponseText res = CallUtils.callUrlHttpPost(staticServer.getPlatformDataUrl()+"tagSub/relateTagSubList",vo);
                    JSONObject jsonObject = JSON.parseObject(res.getData());
                    List<CmsTagSubVO> subList = JSON.parseArray(jsonObject.get("data").toString(), CmsTagSubVO.class);
                    mapActivity.put("subList", subList);
                    //评论列表
                    if(activity.getCommentList()!=null){
                    	mapActivity.put("commentCount", activity.getCommentCount());
                        mapActivity.put("commentList", activity.getCommentList());
                    }
                    //点赞信息
                    mapActivity.put("wantCount", activity.getWantCount() != null ? activity.getWantCount() : "");
                    mapActivity.put("activityIsWant", activity.getActivityIsWant());
     ...
     */
    @Override
    public String queryEventDateActivityCount(String startDate, String endDate, String version) throws Exception {
        ...
        while (CompareTime.timeCompare1(startDate, endDate) <= 0) {
            ...
            int count = 0;
            if(version != null && version.equals("3.6")){	//3.6版本加入采集活动
            	count = activityMapper.queryCultureCalendarActivityCount(map);
            }else{
            	count = activityMapper.queryAppEveryDateActivityCount(map);
            }
        ...
        try {
            ...
            if (activityList != null && activityList.size() > 0) {
                ...
                for (CmsActivity activity : activityList) {
                    ...
                    activityMap.put("activityPayPrice", activity.getActivityPayPrice()!= null ? activity.getActivityPayPrice():"");
                    activityMap.put("activityIsFree", activity.getActivityIsFree() != null ? activity.getActivityIsFree() : "");
                    activityMap.put("sysNo", activity.getSysNo() != null ? activity.getSysNo() : "");
                    activityMap.put("sysId", activity.getSysId() != null ? activity.getSysId() : "");
                    activityMap.put("activitySite", activity.getActivitySite() != null ? activity.getActivitySite() : "");
                    activityMap.put("activityAddress", activity.getActivityAddress() != null ? activity.getActivityAddress() : "");
                    if (activity.getActivityIsReservation() == 2) {
                        if (activity.getSpikeType() == 1) {
                    ...
                    if (StringUtils.isNotBlank(activity.getVenueName())) {
                        activityMap.put("activityLocationName", (StringUtils.isNotBlank(activity.getDictName()) ? activity.getDictName() + "." : "") +
                                activity.getVenueName());
                    } else {
                    	activityMap.put("activityLocationName", (StringUtils.isNotBlank(activity.getDictName()) ? activity.getDictName() + "." : "") +
                                activity.getActivityAddress());
                    ...
                    JSONObject jsonObject = JSON.parseObject(res.getData());
                    List<CmsTagSubVO> subList = JSON.parseArray(jsonObject.get("data").toString(), CmsTagSubVO.class);
                    activityMap.put("subList", subList);
     ...
     */
    @Override
    public String queryHisActivityListByVenueId(String venueId, PaginationApp pageApp) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("venueId", venueId);
        //数据状态
        map.put("activityIsDel", Constant.NORMAL);
        ...
        List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
        try {
            List<CmsActivity> activityList = activityMapper.queryHisActivityListByVenueId(map);
            if (activityList != null && activityList.size() > 0) {
                String staticUrl = staticServer.getStaticServerUrl();
                for (CmsActivity activity : activityList) {
                    Map<String, Object> activityMap = new HashMap<String, Object>();
            ...
            logger.error("app根据展馆id获取历史活动列表出错!", e);
        }
        return JSONResponse.toAppResultFormat(0, listMap);
    }
     ...
     */
    @Override
    public String queryAppCmsActivityListByCondition(String activityArea, String activityType, String activityName, String activityIsReservation,PaginationApp pageApp, String lon, String lat) {
        List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("lat", lat);
        map.put("lon", lon);
        try {
            ...
            if (pageApp != null && pageApp.getFirstResult() != null && pageApp.getRows() != null) {
                map.put("firstResult", pageApp.getFirstResult());
                map.put("rows", pageApp.getRows());
                int total = activityMapper.queryAppCmsActivityListCountAll(map);
                pageApp.setTotal(total);
            }
            List<CmsActivity> activityList = activityMapper.queryAppCmsActivityListByConditionAll(map);
    ...
    public String queryTopActivityList(PaginationApp pageApp, final String tagId, String activityArea,
                                       String activityLocation, String activityIsFree, String activityIsReservation, String sortType,String lon,String lat) {
        if (StringUtils.isBlank(tagId)) {
            return JSONResponse.commonResultFormat(10107, "标签id缺失", null);
        }
        ...
        //如果按类别搜不到，重新按标签搜
        if(list.size()==0&&StringUtils.isNotBlank(tagId)&&!tagId.equals("0")){
        	map.remove("tagId");
        	map.put("tagSubId", tagId);
        	list = activityMapper.queryAppTopActivityList(map);
        }
        List<Map<String, Object>> listMap = this.getAppCmsActivityListResult(list, staticServer);
        if (CollectionUtils.isEmpty(listMap)) {
            listMap = new ArrayList<Map<String, Object>>();
    ...
    public String queryAppHistoryActivityList(PaginationApp pageApp, String userId) {
        ...
        List<Map<String, Object>> listMap = this.makeAppCmsActivityListResult(list, staticServer,userId);
        if (CollectionUtils.isEmpty(listMap)) {
            listMap = new ArrayList<Map<String, Object>>();
        }
        return JSONResponse.toAppResultFormat(100, listMap);
    ...
    public String queryAppMonthActivityList(PaginationApp pageApp, String userId, String startDate, String endDate) {
        ...
        List<Map<String, Object>> listMap = this.makeAppCmsActivityListResult(activityList, staticServer,userId);
        return JSONResponse.toAppResultFormat(100, listMap);
    }

    /**
     * why3.5 app所有活动列表返回数据业务处理(目前只有微网站日历功能使用)(3.6加入采集活动)
     */
    private List<Map<String, Object>> makeAppCmsActivityListResult(List<CmsActivity> activityList, StaticServer staticServer, String userId) {
        List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
        try {
            if (CollectionUtils.isNotEmpty(activityList)) {
                for (CmsActivity activity : activityList) {
                	if(activity.getType() == null || activity.getType() == 1){		//体系内活动
                		Map<String, Object> mapActivity = new HashMap<String, Object>();
                        ...
                        if (StringUtils.isNotBlank(activity.getActivityIconUrl())) {
                        	if(activity.getActivityIconUrl().indexOf("http:")>-1){
                        		mapActivity.put("activityIconUrl",activity.getActivityIconUrl());
                        	}else{
                        		mapActivity.put("activityIconUrl",staticServer.getStaticServerUrl() + activity.getActivityIconUrl());
                        	}
                        }else{
                        	mapActivity.put("activityIconUrl","");
                        }
                        mapActivity.put("sysNo", activity.getSysNo() != null ? activity.getSysNo() : "");
                        mapActivity.put("sysId", activity.getSysId() != null ? activity.getSysId() : "");
    			        ...
    			        mapActivity.put("type", 1);
    			        listMap.add(mapActivity);
    				}else if(activity.getType() == 2){	//采集活动
    					Map<String, Object> map = new HashMap<String, Object>();
    					map.put("userId", userId);
    					map.put("gatherId", activity.getActivityId());
    					CmsGather cmsGather = cmsGatherMapper.queryGatherByCondition(map).get(0);
    					Map<String, Object> mapGather = CommonUtil.transBean2Map(cmsGather);
    					mapGather.put("type", 2);
    					listMap.add(mapGather);
    				}
                }
            }
            return listMap;
	...
	@Override
	public String queryCultureCalendarList(PaginationApp pageApp, String selectDate, String activityType, String userId,String lon, String lat) {
		try {
			List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
			Map<String, Object> map = new HashMap<String, Object>();
			if (pageApp != null && pageApp.getFirstResult() != null && pageApp.getRows() != null) {
			...
			List<CmsActivity> list = activityMapper.queryCultureCalendarList(map);
			
			map.remove("firstResult");
			map.remove("rows");
			for(CmsActivity cmsActivity : list){
				if(cmsActivity.getType() == 1){		//体系内活动
					map.put("activityId", cmsActivity.getActivityId());
					map.put("startDate", selectDate);
			        map.put("endDate", selectDate);
			        List<CmsActivity> activityList = activityMapper.queryAppActivityCalendarList(map);
			        if(activityList.size()>0){
			            Map<String, Object> mapActivity = getAppCmsActivityListResult(activityList, staticServer).get(0);
				        mapActivity.put("type", 1);
				        listMap.add(mapActivity);
			        }
				}else if(cmsActivity.getType() == 2){	//采集活动
					map.put("gatherId", cmsActivity.getActivityId());
					CmsGather cmsGather = cmsGatherMapper.queryGatherByCondition(map).get(0);
					Map<String, Object> mapGather = CommonUtil.transBean2Map(cmsGather);
					mapGather.put("type", 2);
					listMap.add(mapGather);
	...
	@Override
	public String queryMyCultureCalendarList(PaginationApp pageApp, String userId, String startDate, String endDate) {
		try {
			List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
			Map<String, Object> map = new HashMap<String, Object>();
			if (pageApp != null && pageApp.getFirstResult() != null && pageApp.getRows() != null) {
			...
			map.put("endDate", endDate);
			map.put("userId", userId);
			List<CmsActivity> list = activityMapper.queryMyCultureCalendarList(map);
			...
			listMap = this.makeAppCmsActivityListResult(activityList, staticServer,userId);
			if (CollectionUtils.isEmpty(listMap)) {
			    listMap = new ArrayList<Map<String, Object>>();
			}
			return JSONResponse.toAppResultFormat(200, listMap);
	...
	@Override
	public String queryActivityListTagSub() {
		List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
		String []tagSubIds = staticServer.getActTagSubIds().split(",");
		if(staticServer.getActTagSubIds().length() > 0){
			for(int i=0;i<tagSubIds.length;i++){
				Map<String, Object> map = new HashMap<String, Object>();
				CmsTagSub cmsTagSub = cmsTagSubService.queryById(tagSubIds[i]);
				if(cmsTagSub == null) continue;
				map.put("title", cmsTagSub.getTagName());
				map.put("tagSubId", tagSubIds[i]);
				listMap.add(map);
			}
			return JSONResponse.toAppResultFormat(200, listMap);
		}else{
			return JSONResponse.toAppResultFormat(300, "标签未设置");
		}
	}

    /**
     * app获取首页活动标签列表
     * @param activityType 活动主题
     * @return
     */
    @Override
    public List<CmsActivity> queryActivityThemeByCode(String activityType) {
        List<CmsActivity> activityThemeList = activityMapper.queryActivityThemeByCode(activityType);
     ...
     */
    @Override
    public String queryCultureMapActivityListById(String venueId, PaginationApp pageApp) {
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("venueId", venueId);
        //数据状态
        map.put("activityIsDel", Constant.NORMAL);
        ...
        List<Map<String, Object>> listMap = new ArrayList<Map<String, Object>>();
        try {
            List<CmsActivity> activityList = activityMapper.queryCultureMapActivityListById(map);
            if (activityList != null && activityList.size() > 0) {
                String staticUrl = staticServer.getStaticServerUrl();
                for (CmsActivity activity : activityList) {
                    Map<String, Object> activityMap = new HashMap<String, Object>();
