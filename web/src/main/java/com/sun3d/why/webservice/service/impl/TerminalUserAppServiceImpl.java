...
public class TerminalUserAppServiceImpl implements TerminalUserAppService {
        ...
        try {
            ...
            if (mulFile == null) {
                ...
                if(Integer.valueOf(uploadType)==1 || Integer.valueOf(uploadType)==4) {
                    ...
                    //阿里上传
                    AliImageUtils.uploadByInputStream(appFileUrl,newImgName+imgSuffix);
                }
    ...
    public String editTerminalUserById(CmsTerminalUser updateTerminalUser,CmsTerminalUser terminalUser) throws ParseException {
        String json="";
        int count=0;
        if(updateTerminalUser!=null && terminalUser!=null) {
            ...
            if (terminalUser.getUserSex()!=null && terminalUser.getUserSex()!=0) {
                updateTerminalUser.setUserSex(Integer.valueOf(terminalUser.getUserSex()));
            }
            if (StringUtils.isNotBlank(terminalUser.getUserBirthStr())) {
                SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
                updateTerminalUser.setUserBirth(format.parse(terminalUser.getUserBirthStr()));
            }
            if(StringUtils.isNotBlank(terminalUser.getUserEmail())){
                updateTerminalUser.setUserEmail(terminalUser.getUserEmail());
            }
            if(StringUtils.isNotBlank(terminalUser.getUserTelephone())){
   ...
   /*     Map<String, Object> param = new HashMap<String, Object>();
        ...
        // 已经发送过(未激活的号码已经存在)
        if (terminalUser!=null) {
        	if(terminalUser.getLastSendSmsTime()==null)
			...
			if(openId!=null && StringUtils.isNotBlank(openId)){
			    ...
			    if(flag>0){//将原来第三方登录账号添加手机
			        ...
			        if(terminalUser!=null){
			            ...
			            if (count > 0) {
			            	userIntegralDetailService.registerAddIntegral(terminalUserNew.getUserId());
			                json= JSONResponse.commonResultFormat(0, "绑定第三方账号成功!", null);
			            } else {
			                json=JSONResponse.commonResultFormat(1,"绑定第三方账号失败!",null);
			            }
	...
	public String userAuth(String userId, String nickName,String userTelephone, String code, String idCardNo, String idCardPhotoUrl,String userEmail) {
        ...
        String userCardNo= tuser.getUserCardNo();
        
        if(idCardNo!=null && !StringUtils.equals(idCardNo, userCardNo)){
        	
      	  Map<String, Object> map = new HashMap<>();
            
            map.put("userCardNo", idCardNo);
            
           int i= userMapper.queryTerminalUserIsExists(map);
            
           if(i>0){
        	   json = JSONResponse.commonResultFormat(13120, "身份证已被绑定，如有疑问请联系客服!", null);
        	   return json;
           }
        ...
        jsonData.put("userTelephone", userTelephone);
        jsonData.put("idCardNo", idCardNo);
        jsonData.put("userEmail", userEmail);
        jsonData.put("userType", tuser.getUserType().toString());
        
  		CmsTerminalUser terminalUser = JSON.parseObject(jsonData.toString(), CmsTerminalUser.class);
  		CmsTerminalUser updateTerminalUser = queryTerminalUserByUserId(terminalUser.getUserId());
  		String res = null;
		try {
			res = editTerminalUserById(updateTerminalUser, terminalUser);
...
//        	
	...
	@Override
	public String addUserWantgo(String relateId, String userId, Integer type) {
		int flag = 0;
        int status = 0;
        CmsUserWantgo userWantgo = new CmsUserWantgo();
        if (relateId != null && StringUtils.isNotBlank(relateId)) {
            userWantgo.setRelateId(relateId);
        }
        ...
        if (status > 0) {
            return JSONResponse.commonResultFormat(14111, "该用户已点赞,不可重复操作", null);
        }
        userWantgo.setSid(UUIDUtils.createUUId());
        userWantgo.setCreateTime(new Date());
        userWantgo.setRelateType(type);
        flag = cmsUserWantgoMapper.addUserWantgo(userWantgo);
        if (flag > 0) {
            return JSONResponse.commonResultFormat(0, "用户点赞成功", null);
        } else {
            return JSONResponse.commonResultFormat(1, "用户点赞失败", null);
        }
	}

	@Override
	public String deleteUserWantgo(String relateId, String userId) {
		CmsUserWantgo userWantgo = new CmsUserWantgo();
        ...
        if (flag > 0) {
            return JSONResponse.commonResultFormat(0, "用户取消点赞成功", null);
        } else {
            return JSONResponse.commonResultFormat(1, "用户取消点赞失败", null);
        }
	}
}
