...
+ (void)setCornerRadius:(CGFloat)cornerRadius{
    [self sharedView].radius = cornerRadius;
}
...
- (void)showProgress:(float)progress status:(NSString*)string{
    [SVProgressHUD dismissLoadingView];//让加载动画消失
                     ...
                     completion:^(BOOL finished){
                         ...
                         if(strongSelf){
                             if(strongSelf.alpha == 0.0f || strongSelf.hudView.alpha == 0.0f){
                                 ...
                                 //FBLOG(@"%@", [UIApplication sharedApplication].windows);
                                 //FBLOG(@"keyWindow = %@", [[[UIApplication sharedApplication] delegate] window]);
                             }
                         }
                     }];
}
...
+ (id)sharedLoadingView:(BOOL)isInteractive
{
    ...
    CustomLoadingView *sharedView = [window viewWithTag:19345];
    if (!sharedView) {
        if (isInteractive) {
            sharedView = [[CustomLoadingView alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
        }else{
            sharedView = [[CustomLoadingView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight)];
        }
        
        sharedView.tag = 19345;
        [window addSubview:sharedView];
        sharedView.center = CGPointMake(kScreenWidth*0.5, kScreenHeight*0.5-25);
    }
    return sharedView;
}
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        MYMaskView *maskView = [MYMaskView maskViewWithBgColor:COLOR_IWHITE frame:CGRectMake(0, 0, 100, 100) radius:15];
        [self addSubview:maskView];
        maskView.center = CGPointMake(self.width*0.5, self.height*0.5);
        
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 15, 45, 45)];
        imageView.backgroundColor = [UIColor whiteColor];
        static UIImage *gifImage;
        if (!gifImage) {
            NSData *gifData = [NSData dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"loadingImage" ofType:@"gif"]];
            gifImage = [UIImage sd_animatedGIFWithData:gifData];
        }
        ...
        //标题
        CGFloat labelWidth = [UIToolClass textWidth:_titleArray[2] font:FontYT(15)];
        self.loadLabel = [[UILabel alloc] initWithFrame:CGRectMake((maskView.width-labelWidth)*0.5, 0, maskView.width, [UIToolClass fontHeight:FontYT(15)])];
        self.loadLabel.font = FontYT(15);
        self.loadLabel.textColor = kDeepLabelColor;
