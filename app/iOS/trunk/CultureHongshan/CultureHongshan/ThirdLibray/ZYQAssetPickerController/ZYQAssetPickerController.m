...
-(void)drawRect:(CGRect)rect
{
    ...
    [self.text drawInRect:CGRectMake(rect.size.width - titleSize.width - 2, (height - 12) / 2, kThumbnailLength, height-(height - 12) / 2.0f -5) withAttributes:attributes];
...
+ (void)initialize
{
    checkedIcon     = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"ZYQAssetPicker.Bundle/Images/%@@2x.png",(!IOS_VERSION_GREATER_OR_EQUAL(7.0f)) ? @"AssetsPickerChecked~iOS6" : @"AssetsPickerChecked"]]];
...
#pragma -mark 图片提示
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"您选择的图片达到上限了!" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alertView show];
...
#pragma -mark 将图片添加到_indexPathsForSelectedItems
