...
- (void)topBarHidden:(BOOL)topBarHidden
{
    if (topBarHidden == YES) {
...
{
    [self.videoControl.shareButton addTarget:self action:@selector(shareButtonClick) forControlEvents:UIControlEventTouchUpInside];
...
{
    //[self dismiss];
    [self.myDelegate displayBar];//返回VideoPlayer 显示Bar
...
{
    [self.myDelegate hiddenBar];//点击全屏 隐藏Bar
...
- (void)setDeviceOrientationLandscapeRight{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        self.frame = frame;
        FBLOG(@"1111111 frame = %@",NSStringFromCGRect(frame));
...
- (KrVideoPlayerControlView *)videoControl
{
    if (!_videoControl) {
        ...
        _videoControl.volumeView.frame = CGRectMake(-60, CGRectGetMaxY(_videoControl.frame)+170, 200, 20);
        _videoControl.volumeView.transform = CGAffineTransformMakeRotation(M_PI * 1.5);
        _videoControl.volumeView.hidden = YES;
    }
    return _videoControl;
}
- (void)volumeHidden:(BOOL)volumeHidden
{
    if (volumeHidden == YES) {
        _videoControl.volumeView.hidden = YES;
        
    }else{
        _videoControl.volumeView.hidden = NO;
    }
}
...
+ (UIImage*) thumbnailImageForVideo:(NSURL *)videoURL atTime:(NSTimeInterval)time
{
    ...
    if (!thumbnailImageRef)
        FBLOG(@"thumbnailImageGenerationError %@", thumbnailImageGenerationError);
