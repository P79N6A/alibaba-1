...
import com.sun3d.why.dao.TicketMachineVenueMapper;
   ...
   //     int status = 0;
      ...
      /*  Map<String, String> map = new HashMap<String, String>();
        ...
        if (cmsActivityOrder != null) {
            ...
            String dateTime=cmsActivityOrder.getEventDateTime().substring(0, cmsActivityOrder.getEventDateTime().lastIndexOf(" "));
...
//            if(statusDate1 <= 0){
                ...
                listMap.add(map);
                json=JSONResponse.toAppResultFormat(0,"1",listMap);
            }else{
...
//        }
     ...
     */
    @Override
    public String takeAppActivityValidateCode(String orderValidateCode,String seat,int index,String machineCode) throws Exception {
        String json = "";
        int printCount= 0;
        int flag = 0;
        int status = 0;
        ...
        CmsActivityOrder cmsActivityOrder = cmsActivityOrderMapper.queryActvityOrderByValidateCode(param);
//        printCount=cmsActivityOrder.getPrintTicketTimes();
        if (cmsActivityOrder != null) {
                String[] orderCodes = cmsActivityOrder.getSeats().split(",");
                for (int j = 0; j < orderCodes.length; j++) {
                    String orderCode = orderCodes[j];
                    ...
                    for (int i = 0; i < seats.length; i++) {
                        if (seats[i].equals(orderCode)) {
                            ...
                            cmsActivityOrderDetail.setUpdateTime(new Date());
                            cmsActivityOrderDetail.setSeatVal(seats[i]);
                            cmsActivityOrderDetail.setMachineCode(machineCode);
                            flag = cmsActivityOrderDetailMapper.editCmsActivityOrderDetailByStatus(cmsActivityOrderDetail);
                        }
                    }
                }
                if (flag > 0) {
                    ...
                    String venueId=cmsActivityOrder.getVenueId();
//                    
//                    String codecnysg="cnysg1,cnysg2,cnysg3";
//                    
//                    String codeqyg="qyg1,qyg2";
//                    
//                    String codezhysg="zhysg";
//                    
//                    // 长宁文化艺术中心
//                    String venuecnysg="561d99fbd51f44bba25b287843c8d023";
//                    // 中华艺术宫 场馆
//                    String venuezhysg="2f579b2d7acd497f9ded78df0542d182";
//                    // 上海市群众艺术馆
//                    String venueqyg="5e8739f0511b4caeb05c974273b83b96";
//                    
                    boolean f=false;
                    
                    if(StringUtils.isNotBlank(machineCode)){
                    	List<TicketMachineVenue> venueList=ticketMachineVenueMapper.selectTicketMachineVenueByModel(machineCode, venueId);
                    ...
                    if(f){
                    	...
                    	Integer orderVotes=cmsActivityOrder.getOrderVotes();
                    	if(StringUtils.isNotBlank(costTotalCredit)&&orderVotes!=null){
                    		int totalCredit=0;
                    		try {
                    			totalCredit= Integer.parseInt(costTotalCredit);
                    			
                    			if(totalCredit>0){
                        			int credit=totalCredit/orderVotes*printCount;
                        			
                        			//添加积分
                        			UserIntegralDetail userIntegralDetail = new UserIntegralDetail();
                        			userIntegralDetail.setIntegralChange(credit);
                        			userIntegralDetail.setChangeType(0);
                        			userIntegralDetail.setIntegralFrom("取票机"+machineCode+"成功打印票返回积分");
                        			userIntegralDetail.setIntegralType(IntegralTypeEnum.VERIFICATION.getIndex());
                        			userIntegralDetail.setUserId(cmsActivityOrder.getUserId());
                    ...
                    if (count == 0) {
                        ...
                        cmsActivityOrder.setCostTotalCredit(null);
                        status = cmsActivityOrderMapper.editActivityOrder(cmsActivityOrder);
                        if (status > 0) {
                            //多个座位信息
                            json=ActivitySeatTemplate.toActivitySeatTemplate(cmsActivityOrder,seat,index,staticServer,basePath);
                        }
                     } else {
                            //单个座位信息
                            json=ActivitySeatTemplate.toActivitySeatTemplate(cmsActivityOrder,seat,index, staticServer, basePath);
     ...
     * @return
     */
    public CmsActivityOrder queryValidateCode(String orderValidateCode){
        Map<String,Object> param=new HashMap<String, Object>();
        if(orderValidateCode!=null && StringUtils.isNotBlank(orderValidateCode)){
            param.put("orderValidateCode",orderValidateCode);
        }
