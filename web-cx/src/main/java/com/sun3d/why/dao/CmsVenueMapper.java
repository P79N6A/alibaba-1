...
public interface CmsVenueMapper {
	...
	@Select("SELECT v.VENUE_ID venueId,v.VENUE_NAME venueName,v.VENUE_ICON_URL venueIconUrl,v.VENUE_ADDRESS venueAddress from cms_member m join cms_member_relation r on m.id=r.member_id and r.relation_type=1 join cms_venue v on r.relation_id = v.VENUE_ID WHERE m.id=#{0} and v.VENUE_IS_DEL=1 and v.VENUE_STATE=6 order by v.VENUE_CREATE_TIME desc limit #{1}")
	List<CmsVenue> queryVenueUnion(String member,Integer count);

	@Select("SELECT v.VENUE_ID venueId,v.VENUE_NAME venueName,v.VENUE_ICON_URL venueIconUrl,v.VENUE_ADDRESS venueAddress from cms_member m join cms_member_relation r on m.id=r.member_id and r.relation_type=1 join cms_venue v on r.relation_id = v.VENUE_ID WHERE m.id=#{0} and v.VENUE_IS_DEL=1 and v.VENUE_STATE=6 order by v.VENUE_CREATE_TIME desc limit #{1},6")
	List<CmsVenue> queryVenueUnionPage(String member,Integer index);
   ...
   //前台获取场馆数据
	 ...
	 * @return
	 */
	Integer queryIsWantGo(Map<String, Object> map);
	
	
	List<CmsVenue> queryVenueByType(Map<String, Object> map);

	List<CmsVenue> queryPcnewVenue(Map map);
