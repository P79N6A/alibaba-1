...
<head>
    ...
    <script type="text/javascript" src="${path}/STATIC/js/index/userCenter/editPublicActivity.js"></script>
...
<div class="crumb"><i></i>您所在的位置： <a href="#">个人主页</a> &gt; 编辑活动</div>
        ...
        <div class="publish">
            <table width="100%" border="0">
                <tr>
                    ...
                    <td class="td-input" id="activityNameLabel"><input type="text" value="<c:out escapeXml="true" value="${activity.activityName}"/>" name="activityName" id="activityName" placeholder="请输入活动名称" class="input-text w330"/></td>
                </tr>
                <tr>
...
<%--                            <div class="img fl">
                         ...
                         <span>
                            <img id="imgHeadPrev"  width="120" height="80">
                          &lt;%&ndash;  <span id="fileContainer"></span>
                            <input type="file" name="file" id="file">&ndash;%&gt;
                         </span>--%>
                        ...
                        <div class="btn controls-box" style="margin-top: 40px;">
                            <div class="controls">
                                <input type="file" name="file" id="file">
                            </div>
                ...
                <tr>
                    ...
                    <td class="td-input" id="activityAddressLabel"><input type="text" value="<c:out escapeXml="true" value="${activity.activityAddress}"/>" name="activityAddress" id="activityAddress" placeholder="请输入活动地址" class="input-text w330"/></td>
                </tr>
                <tr>
                    <td width="130" class="td-title"><span class="red">*</span>地图坐标</td>
                    <td class="td-input td-coordinate" id="LonLabel">
                        <input type="text" name="activityLon" id="activityLon" value="${activity.activityLon}" class="input-text w130" placeholder="X"/><span class="txt">X</span>
                        <input type="text" name="activityLat" id="activityLat" value="${activity.activityLat}" class="input-text w130" placeholder="Y"/><span class="txt">Y</span>
                ...
                <tr>
                    ...
                    <td class="td-time" id="activityStartTimeLabel">
                        <div class="start w172">
                            <input type="text" value="${activity.activityStartTime}" id="activityStartTime" name="activityStartTime" placeholder="开始时间" readonly/>
                            <i class="data-btn start-btn"></i>
                        </div>
                        <span class="txt">至</span>
                        <div class="end w172">
                            <input type="text" value="${activity.activityEndTime}" id="activityEndTime" name="activityEndTime" placeholder="结束时间" readonly/>
                            <i class="data-btn end-btn"></i>
                        </div>
                        <script type="text/javascript">
                            $(function(){
                                $(".start-btn").on("click", function(){
                                    WdatePicker({el:'activityStartTime',minDate:'%y-%M-{%d}',dateFmt:'yyyy-MM-dd',doubleCalendar:true,maxDate:'#F{$dp.$D(\'activityEndTime\')}',position:{left:-19,top:7},isShowClear:false,isShowOK:true,isShowToday:false,onpicked:pickedStartFunc,disabledDates:['2015-11-11','2015-11-15']})
                                });
                            ...
                            <div id="put-ticket-list" >
                             <c:forEach items="${activityEventTimes}"  var="activityEventTime" varStatus="varStatus">
                                ...
                                <div class="ticket-item" id="activityTimeLabel${varStatus.index+1}">
                                    <input type="text" value="${fn:split(startTime, ':')[0]}" data-type="hour" id="startHourTime${varStatus.index+1}" name="eventStartHourTime" class="input-text" maxlength="2" value="00"/><em>:</em>
                                    <input type="text" value="${fn:split(startTime, ':')[1]}" data-type="minute" id="startMinute${varStatus.index+1}" name="eventStartMinuteTime" class="input-text" maxlength="2" value="00"/><span class="zhi">至</span>
                                    <input type="text" value="${fn:split(endTime, ':')[0]}" data-type="hour" id="endHourTime${varStatus.index+1}" name="eventEndHourTime" class="input-text" maxlength="2" value="00"/><em>:</em>
                                    <input type="text" value="${fn:split(endTime, ':')[1]}" data-type="minute" id="endMinute${varStatus.index+1}" name="eventEndMinuteTime" class="input-text" maxlength="2" value="00"/>
                                    <c:if test="${varStatus.index == 0}" ><a href="javascript:void(0)" class="timeico add-btn"></a> </c:if>
                                    <c:if test="${varStatus.index > 0}" ><a href="javascript:void(0)" class="timeico del-btn"></a> </c:if>
                                </div>
                             </c:forEach>
                            </div>
                        </div>
                ...
                <tr>
                    ...
                    <td class="td-input" id="activityTelLabel"><input type="text" value="<c:out escapeXml="true" value="${activity.activityTel}"/>" id="activityTel" name="activityTel" placeholder="请输入电话" class="input-text w330"/></td>
                </tr>
                <tr>
                    <td width="130" class="td-title"><span class="red">*</span>入场方式</td>
                    <td class="td-input td-way">
                        <label><input type="radio" onclick="showCount(1)" <c:if test="${activity.activityIsReservation == 1}" > checked </c:if>  name="activityIsReservation" value="1"/><em>不可预订</em></label>
                        <label><input type="radio" onclick="showCount(2)" <c:if test="${activity.activityIsReservation == 2}" > checked </c:if>  name="activityIsReservation" value="2"/><em>自由入座</em></label>
                        <div class="extra" <c:if test="${activity.activityIsReservation == 1}" >  style="display: none" </c:if> id="showCount">
                            <em>每场售票数</em>
                            <input onblur="getTotalTicketCount();" type="text" id="eventCount" name="eventCount" class="input-text w120" value="${activity.eventCount}" onkeyup="this.value=this.value.replace(/\D/g,'')"/>
                ...
                <tr>
                    ...
                    <td class="td-content" id="activityMemoLabel">
                        <div class="editor-box">
                            <textarea  name="activityMemo" id="activityMemo">${activity.activityMemo}</textarea>
                        </div>
                    </td>
                </tr>
                <tr>
