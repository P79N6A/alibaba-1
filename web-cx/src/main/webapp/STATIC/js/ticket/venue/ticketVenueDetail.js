...
function searchVenueList(){
    ...
    window.location.href = "../ticketVenue/venueList.do?keyword="+encodeURI(venueName);
}
...
function loadAntiqueList(){
    ...
    $.post("../ticketVenue/antiqueList.do",data ,
        function(data) {
            var antiqueHtml = "";
            if(data != null && data != ""){
...
function loadRoomList(){
    var venueId = $("#venueId").val();
    var data = {"venueId":venueId};
    $.post("../ticketVenue/roomList.do",data ,
        function(data) {
            var roomHtml = "";
            if(data != null && data != ""){
                //当活动室数量大于6个时才显示查看更多
