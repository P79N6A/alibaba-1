...
function showDetailImg(){
    var url = $("#detailImg").attr("data-url");
    $("#detailImg").find("img").attr("src", getImgUrl(url));
}
...
function changeClass(){
    ...
    $.ajax({
        type:"post",
        url:"../mc/addNewsLove.do",
        data:{loveUserId:loveUserId,newsId:$("#newsId").val()},
        dataType: "text",
...
function addComment(){
    ...
    $.ajax({
        ...
        data:{commentUserId:$("#userId").val(),commentRkId:$("#newsId").val(),commentRemark:$("#commentRemark").val(),commentType:11},
        dataType: "json",
        cache:false,//缓存不存在此页面
        async: false,//异步请求
...
function loadCommentList(){
    ...
    $.post("../mc/commentList.do",{"commentType" : 11,"commentRkId" :newsId} ,
        function(data) {
            var commentHtml = "";
            if(data != null && data != ""){
